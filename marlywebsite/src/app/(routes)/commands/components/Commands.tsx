            ...
            <div className="2xl:container 2xl:mx-auto -mt-2 px-20 md:px-[8vw] 2xl:px-52 2xl:py-4">
                ...
                <div className="flex flex-row justify-between items-center -mx-10 sm:-mx-0">
                    <div className="flex justify-center items-center space-x-2">
                        <div className="rounded-full p-3 border border-marly-card-border bg-marly-200">
                            <TerminalSquareIcon size="25" className="text-marly-main" />
                        </div>
                    ...
                    <div className="flex flex-row items-center gap-4 mt-2 sm:mt-0">
                        <div
                            className="flex items-center justify-center rounded-2xl border border-marly-card-border bg-marly-200 hover:bg-marly-300 cursor-pointer p-3"
                            onClick={() => setSearchMenuOpen(true)}>
                            <Search size="25" className="text-marly-main" />
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex flex-row justify-between items-center">
                        ...
                        <div className="flex flex-row items-center gap-4 mt-2 sm:mt-0">
                            <div className="flex flex-col items-center">
                                <span className="text-marly-main font-medium text-sm mb-2 hidden sm:block">Search</span>
                                <div
                                    className="flex items-center justify-center rounded-2xl border border-marly-card-border bg-marly-200 hover:bg-marly-300 cursor-pointer p-3"
                                    onClick={() => setSearchMenuOpen(true)}>
                                    <Search size="25" className="text-marly-main-200" />
                                </div>
                            </div>
                        </div>
                    </div>
...
const Command = ({
    ...
    return (
        <div
            id={name}
            className="flex flex-col py-6 rounded-3xl bg-marly-200 border transition-shadow duration-200 ease-linear border-marly-card-border text-white">
            <div className="relative h-full flex flex-col justify-between">
                <div className="px-6">
                    ...
                    <p className="text-sm text-marly-secondary font-medium pb-[5%]">
                        {description}
                    </p>
                </div>
                <hr className="border-t border-marly-card-border w-full" />
                <div>
                    <div className="px-6 pt-[5%] flex flex-col gap-4">
                        <div>
                            <p className="text-sm tracking-wide text-marly-main font-medium">
                                arguments
                            </p>
                            <div className="flex flex-wrap gap-2 mt-3">
                                {args.length === 0 ? (
                                    <p className="text-[#D6D6D6] text-sm py-1 px-2 rounded-lg bg-marly-300">
                                        None
                                    </p>
                                ) : (
                                    args.map((arg: string, index: number) => (
                                        <p
                                            key={index}
                                            className="text-[#D6D6D6] text-sm py-1 px-2 rounded-lg bg-marly-300">
                                            {arg.replaceAll("_", " ")}
                                        </p>
                                    ))
                                )}
