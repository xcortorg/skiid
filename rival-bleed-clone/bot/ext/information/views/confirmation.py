...
async def confirm(
    ...
    msg: discord.Message,
    invoker: discord.User = None,
    invoked: discord.User = None,
    timeout: int = 180,
):
...
class Confirm(discord.ui.View):
    ...
    @discord.ui.button(style=ButtonStyle.grey, emoji=CONFIG["emojis"]["success"])
    async def confirm(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        await self.confirmation(interaction, True)

    @discord.ui.button(style=ButtonStyle.grey, emoji=CONFIG["emojis"]["fail"])
        ...
        if confirm:
            if not interaction.user.id == self.invoker.id:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        description=f"{interaction.client.config['emojis']['warning']} {interaction.user.mention}: **You cannot interact with this**",
