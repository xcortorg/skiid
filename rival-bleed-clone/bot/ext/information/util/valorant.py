...
async def valorant(ctx, username: str):
    ...
    if response.status == 404:
        raise CommandError(f"Couldn't find an account for `{username}#{tag}`")
    ...
    if response.status == 404:
        raise CommandError(f"Couldn't find any matches for `{username}#{tag}`")
    elif response.status == 429:
        raise CommandError(
            "The **API** is currently **rate limited** - Try again later"
        )
    ...
    if matches:
        embed.add_field(
            ...
            value="\n".join(
                f"> {utils.format_dt(match.started_at, 'd')} {match.status} (`%.2f`)"
