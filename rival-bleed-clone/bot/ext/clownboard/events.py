...
class Events(Cog):
    ...
    async def reaction_logic(self, fmt: str, payload: discord.RawReactionActionEvent):
        ...
        if not (
            clownboard := await self.bot.db.fetchrow(
                "SELECT * FROM clownboard WHERE guild_id = $1",
                guild.id,
            )
        ):
            return self.log("no clownboard")
        if not str(clownboard.emoji) == str(payload.emoji):
            return self.log(
                f"{str(clownboard.emoji)} isnt the same as {str(payload.emoji)}"
            )

        if not ((clownboard_channel := guild.get_channel(clownboard["channel_id"]))):
            return self.log("no channel")
        if not (clownboard_channel.permissions_for(guild.me).send_messages):
        ...
        if clownboard.ignore_entries:
            clownboard.ignore_entries = orjson.loads(clownboard.ignore_entries)
            snowflakes = [member.id, channel.id]
            snowflakes.extend([r.id for r in member.roles])
            for entry in clownboard.ignore_entries:
        ...
        try:
            ...
            self.log(f"clownboard error: {exc}")
            return
    ...
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel):
        ...
        await self.bot.db.execute(
            "DELETE FROM clownboard WHERE guild_id = $1 AND channel_id = $2",
            channel.guild.id,
            channel.id,
        )
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        await self.reaction_logic("clown", payload)

    @Cog.listener()
    async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent):
        await self.reaction_logic("unclown", payload)
    ...
    async def on_raw_reaction_clear(self, payload: discord.RawReactionClearEmojiEvent):
        ...
        clownboard_entry = await self.bot.db.fetchrow(
            "DELETE FROM clownboard_entries WHERE message_id = $1 RETURNING emoji, clownboard_message_id",
            payload.message_id,
        )
        if not clownboard_entry:
            return

        if not (
            clownboard := await self.bot.db.fetchrow(
                "SELECT channel_id FROM clownboard WHERE guild_id = $1 AND emoji = $2",
                guild.id,
                clownboard_entry["emoji"],
            )
        ):
            return
        ...
        with contextlib.suppress(discord.HTTPException):
            await clownboard_channel.delete_messages(
                [discord.Object(id=clownboard_entry["clownboard_message_id"])]
            )
    ...
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent):
        ...
        await self.bot.db.execute(
            "DELETE FROM clownboard_entries WHERE guild_id = $1 AND clownboard_message_id = $2",
            payload.guild_id,
            payload.message_id,
        )
    ...
    async def on_raw_bulk_message_delete(
        ...
        await self.bot.db.execute(
            "DELETE FROM clownboard_entries WHERE guild_id = $1 AND clownboard_message_id = ANY($2::BIGINT[])",
            payload.guild_id,
            list(payload.message_ids),
        )

    async def clown_message(
        self,
        clownboard: Record,
        clownboard_channel: Union[discord.TextChannel, discord.Thread],
        guild: discord.Guild,
        ...
        try:
            return await self._clown_message(
                clownboard, clownboard_channel, guild, channel, member, message_id
            )
        except Exception as e:
            exc = "".join(traceback.format_exception(type(e), e, e.__traceback__))
            self.log(f"clownboard error: {exc}")

    async def _clown_message(
        self,
        clownboard: Record,
        clownboard_channel: Union[discord.TextChannel, discord.Thread],
        guild: discord.Guild,
        ...
        async with lock:
            if channel.is_nsfw() and not clownboard_channel.is_nsfw():
            ...
            if message.author.id == member.id:
                if not clownboard.get("self_star", True):
            ...
            reaction = [
                ...
                if str(reaction.emoji) == clownboard["emoji"]
            ]
            if reaction:
                reaction = reaction[0]
            else:
            ...
            clownboard_message_id = await self.bot.db.fetchval(
                "SELECT clownboard_message_id FROM clownboard_entries WHERE guild_id = $1 AND channel_id = $2 AND message_id = $3 AND emoji = $4",
                guild.id,
                channel.id,
                message.id,
                clownboard["emoji"],
            ...
            content, embed, files = await self.render_clownboard_entry(
                clownboard, reaction, message
            )

            if clownboard_message_id and (
                clownboard_message := clownboard_channel.get_partial_message(
                    clownboard_message_id
                )
            ):
                try:
            ...
            await self.bot.db.execute(
                "INSERT INTO clownboard_entries (guild_id, channel_id, message_id, emoji, clownboard_message_id) VALUES ($1, $2, $3, $4, $5)",
                guild.id,
                channel.id,
                message.id,
                clownboard["emoji"],
    ...
    async def unclown_message(
        ...
        async with lock:
            ...
            if reaction:
                ...
                with contextlib.suppress(discord.HTTPException):
                    await clownboard_channel.delete_messages(
                        [discord.Object(id=clownboard_message_id)]
                    )

                await self.bot.db.execute(
                    "DELETE FROM clownboard_entries WHERE clownboard_message_id = $1",
                    clownboard_message_id,
                )
                return
            ...
            try:
                await clownboard_channel.get_partial_message(
                    clownboard_message_id
                ).edit(
                    content=content,
                )
    ...
    async def render_clownboard_entry(
        self,
        clownboard: Record,
        ...
        if (
            ...
            embed = discord.Embed(
                color=(
                    Color.from_str(clownboard.color)
                    if clownboard.color
