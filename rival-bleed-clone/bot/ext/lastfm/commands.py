...
from lib.classes.flags.lastfm import CollageFlags
...
from lib.classes.processing import shorten
from typing import Optional, List, Union, Dict, Tuple, Any
from lib.classes.managers import ClientSession
from lib.classes import lastfm
from lib.classes.lastfm import Client as LastFMClient
...
from lib.classes.converters import Timeframe, Artist, Album, Track
import discord
import orjson
from datetime import datetime
import urllib
from lib.classes.processing import format_duration
import pytz
from lib.classes.music import soundcloud, itunes
...
class Commands(Cog):
    ...
    async def cog_load(self: "Commands") -> None:
        await self.spotify_client.authorize()

    async def scrape_play_locations(self: "Commands", ctx: Context, data: Munch):
    ...
    async def cog_check(self: "Commands", ctx: Context):
        if not ctx.command:
            return False
        ...
        if getattr(
            ctx,
            "__lastfm",
            getattr(ctx, "_Commands__lastfm", getattr(ctx, "_Context__lastfm", None)),
        ):
            return True
    ...
    @group(
        name="lastfm", aliases=["fm", "lf"], description="", invoke_without_command=True
    )
    ...
    async def lastfm_update(self, ctx: Context) -> Message:
        ...
        elapsed = perf_counter() - start
        log.info(f"Succesfully indexed {username}'s library in {elapsed:.2f}s.")
    ...
    @lastfm.command(
        ...
        example=",lastfm favorites @kuzay",
    )
    ...
    @lastfm.command(name="streak", description="", example=",lastfm streak @kuzay")
    ...
    @lastfm.group(
        ...
        example="lastfm customcommand kuzayfm --public",
    ...
    @customcommand.command(
        ...
        example=",lastfm customcommand remove kuzay",
    )
    @has_permissions(manage_guild=True)
    ...
    @customcommand.group(
        ...
        invoke_without_command=True,
        example=",lastfm customcommand blacklist @kuzay",
    )
    @has_permissions(manage_guild=True)
    ...
    @lastfm.command(
        ...
        example=",lastfm soundcloud @kuzay",
    )
