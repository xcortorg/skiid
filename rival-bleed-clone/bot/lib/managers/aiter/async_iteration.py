...
class asuppress(suppress):
    def __init__(
        self, exceptions: type[BaseException] = (discord.HTTPException,)
    ) -> None:
...
class aiterdict(AsyncIter):
    def __init__(
        ...
        super().__init__(
            iterable.items(), delay, steps, circuit_breaker=circuit_breaker
        )
    ...
    for chunk in chunked_iter(future_like, chunksize):
        ...
        async with taskguard(tasks):
            for coro_or_future in chunk:
                if max_concurrent:
                    tasks.append(
                        asyncio.ensure_future(_await_bounded(coro_or_future, task_sem))
                    )
                else:
                    if isinstance(
                        coro_or_future,
                        (asyncio.Future, distributed.Future, asyncio.Task),
                    ):
                        tasks.append(coro_or_future)
                    else:
                        tasks.append(
                            asyncio.ensure_future(_await_bounded(coro_or_future))
                        )
...
async def aiterforever(
    circuit_breaker=False, loop_delay=30, task_delay=0.01, iterables=None
):
    while True:
