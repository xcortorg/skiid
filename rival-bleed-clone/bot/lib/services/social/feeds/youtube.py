...
class YouTube(Feed):
    ...
    def replacements(self: Self, user: YouTubeChannel, post: FeedEntry) -> dict:
        REPLACEMENTS = {
            "{post.description}": shorten(post.title or "", width=256),
    ...
    async def feed(self: Self) -> NoReturn:
        log.info("Started YouTube Feed!")
        while True:
    ...
    async def get_posts(self: Self, youtube_id: str, records: List[Record]) -> NoReturn:
        ...
        if not feed:
            log.info(f"Couldnt fetch feed for YouTube Channel ID {youtube_id}")
            return
        for item in feed.entries[:3]:
            if await self.redis.sismember(self.key, str(item.yt_videoid)):
                log.info(f"skipping {str(item.yt_videoid)} due to it already have been sent")
                continue
            if datetime.now(timezone.utc) - datetime.fromisoformat(item.published) > timedelta(hours=1):
                log.info(f"skipping {item} due to it being to old")
                self.bot.ytvideo = item
                continue
            await self.redis.sadd(self.key, str(item.yt_videoid))
