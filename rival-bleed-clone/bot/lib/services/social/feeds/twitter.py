...
from ..models.twitter.tweets import BasicUser, Tweet, Tweets
...
class Twitter(Feed):
    ...
    async def feed(self: Self) -> None:
        log.info(f"Started Twitter Feed")
        while True:
    ...
    async def fetch_tweets(self: Self, user_id: int, records: List[Record]) -> None:
        ...
        """
        cached = False
        if pull := await self.bot.redis.get(self.make_key(f"twitter_posts:{user_id}")):
            try:
                data = Tweets(**orjson.loads(pull))
                cached = True
            except:
                pass
        if not cached:
            try:
                ...
                log.info(f"error during tweet fetching: {e}")
            ...
            if not data or not data.user:
                ...
                return
        if not cached:
            await self.bot.redis.set(self.make_key(f"twitter_posts:{user_id}"), orjson.dumps(data.dict()), ex = 60*29)
        ...
        for tweet in reversed(data.tweets[:6]):
            ...
            if await self.redis.sismember(self.key, str(tweet.id)):
                log.info(f"skipping {str(tweet.id)} due to it already have been sent")
                continue
