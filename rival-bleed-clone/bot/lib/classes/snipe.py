...
from .exceptions import SnipeError
        ...
        if isinstance(message, discord.Message):
            ...
            entry: dict = {
                ...
                "author": {
                    "id": message[2].id,
                    "name": message[2].name,
                    "avatar": message[2].display_avatar.url,
                },
            }
        if type.lower() == "snipe":
            ...
            if f"rs-{message[0].channel.id}" not in self.data.keys():
                self.data[f"rs-{message[0].channel.id}"] = deque(maxlen=100)
            else:
                if len(self.data[f"rs-{message[0].channel.id}"]) == 100:
                    self.data[f"rs-{message[0].channel.id}"].pop()
            self.data[f"rs-{message[0].channel.id}"].insert(0, entry)
