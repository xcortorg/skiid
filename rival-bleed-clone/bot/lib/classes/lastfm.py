...
from .managers import BaseModel, ClientSession, CS
from .models import LastfmProfile, LastfmProfileInformation, LastfmProfileLibrary
import hashlib
...
import aiohttp
import socket
from .exceptions import LastFMError
...
async def index(
    ctx: Context, user: Union[Munch, Record]
 ...
 #       if pages < 1: page = 1
        ...
        for i, page in enumerate(range(2, pages+1), start = 0):
            ...
            while True:
                ...
                attempts += 1
                #logger.info(f"index request {attempts} {p}")
                if attempts >= 5:
                     break
                if p:
                     break
            if not p: p = await api_request
            items.extend(p)
...
async def login(session: ClientSession, ctx: Context):
    ...
    link = f'https://www.last.fm/api/auth?api_key={key}&token={token}&cb=https://api.kainu.sh/lastfm?user_id={user_id}'
    try:
        message = await ctx.author.send(embed = Embed(color = 0xd31f27, description = f"""Authorize **rival** to use your Last.fm account [here]({link}). Your library's data will be indexed to power the whoknows commands and other commands.\n\nIf you want to remove your account with **coffin**, run `lastfm logout` and visit your settings on [Last.fm](https://last.fm/settings/applications/) to unauthorize our application"""))
    except:
