...
def get_bar(bot: Client, percentage: int) -> str:
    ...
    bright = bot.config["emojis"]["blue_right"]
    bleft = bot.config["emojis"]["blue_left"]
    blue = bot.config["emojis"]["blue_regular"]
    if percentage > 2:
        bar[0] = bleft
...
class Level:
    ...
    async def get_member_xp(self, ctx: Context, member: Member) -> Embed:
        ...
        embed = (
            Embed(title=f"{str(member)}'s level", url="https://coffin.bot", timestamp = datetime.now())
