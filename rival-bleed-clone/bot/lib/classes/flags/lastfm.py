...
class CollageFlags(FlagConverter, delimiter=" "):
	size: Optional[str] = flag(name = "size", default=None, description="a size in the form of height and width")
	time_period: Optional[TimePeriodConverter] = flag(name = "time-period", aliases = ["timeperiod", "time", "timeframe"], default="overall", description = "the timeperiod of data to add to the collage")
	chart_type: Optional[ChartTypeConverter] = flag(name = "type", aliases = ["chart-type"], default="artist", description = "the type of data to collage")
