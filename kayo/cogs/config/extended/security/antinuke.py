...
class Settings(BaseModel):
    ...
    async def check_threshold(self, bot: swag, member: Member, module: str) -> bool:
        """
    ...
    @classmethod
    async def revalidate(cls, bot: swag, guild: Guild) -> Optional[Self]:
        """
    ...
    @classmethod
    async def fetch(cls, bot: swag, guild: Guild) -> Self:
        """
...
class AntiNuke(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def antinuke_settings(self, ctx: Context) -> Message:
        ...
        return await ctx.send(
            f"i'm sorry for this ugly message but i couldn't design a good embed for this 🥺 duration is in seconds btw\n{config.json(indent=4, exclude={'guild'})}"
        )
