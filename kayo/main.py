...
async def get_prefix(bot: "swag", message: Message) -> List[str]:
...
class swag(Bot):
    ...
    reddit: RedditClient
    version: str = "0.3"
    user_agent: str = f"swag (DISCORD BOT/{version})"
    ...
    async def on_guild_join(self, guild: Guild) -> None:
        ...
        #     await self.notify(
        #         guild,
        #         f"swag is a premium bot and requires a subscription to use - {config.CLIENT.SUPPORT_URL}",
        #     )
        #     with suppress(HTTPException):
        #         await guild.leave()
        #         return
