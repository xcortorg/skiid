...
from discord import (Activity, ActivityType, AllowedMentions, AuditLogEntry,
            ...
            if (
                ...
                if message.mentions_bot(strict=True):
                    if (
                        ...
                        user_prefix_value = (
                            f"\nYour Custom Prefix is `{user_prefix}`"
                            if user_prefix
                            else ""
                        )
                        return await ctx.normal(
                            f"Guild Prefix is `{guild_prefix}`{user_prefix_value}"
                        )
