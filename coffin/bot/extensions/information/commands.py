...
class Information(Cog):
    ...
    @hybrid_command(
        name="status", description="get information on discord's current status"
    )
    ...
    async def credits_(self, ctx: Context):
        embed = (
            Embed()
            .set_footer(
                text=f"{self.bot.user.name} credits â€¢ {self.bot.user.name}.lol"
            )
...
**Files**: {stat.files}""",
    ...
    @command(
        name="itemshop", aliases=["fnshop"], description="View the Fortnite item shop"
    )
    ...
    async def image(self, ctx: Context, *, query: str):
        ...
        try:
            ...
            commands = [
                "cd ~/coffin-rewrite/proxy-server",
                "go run ./add6.go --ipv6 2602:fa48:0:2:: --v6_n 300 --interface eno49",
                "sudo docker container restart ipv6-proxy",
            ]
    ...
    async def google(self, ctx: Context, *, query: str):
        ...
        try:
            ...
            commands = [
                "cd ~/coffin-rewrite/proxy-server",
                "./add6 --ipv6 2602:fa48:0:2:: --v6_n 300 --interface eno49",
                "sudo docker container restart ipv6-proxy",
            ]
            for command in commands:
                await create_subprocess_shell(command)
