...
class Result(BaseModel):
    ...
    cite: Optional[str] = Field(None)  # Added cite field
    description: Optional[str] = Field(None)  # Added description field
...
class Google(BaseModel):
    ...
    @classmethod
    async def search(
        cls, session: ClientSession, query: str, safe: str = "off"
    ) -> Self:
        url = URL.build(
            scheme="https",
            host="www.google.com",
