...
from system.tools.utils import (
    human_join,
    shorten_lower as shorten,
    Plural,
    title_format,
    format_overwrites,
)
...
class Logging(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def logging_ignore_list(self, ctx: Context) -> Message:
        ...
        ignored_targets = [
            f"{target.mention} (`{target.id}`)" for target in ctx.settings.log_ignore
        ]

        return await ctx.autopaginator(
            embed=Embed(title="Logging Exemption"),
            description=ignored_targets,
        )
    ...
    async def log(
        ...
        *,
        ...
        log.debug(
            "Dispatching {event.name} log for {guild} ({guild.id}).",
            event=event,
            guild=guild,
        )
        channel_id = cast(
    ...
    async def log_channel_create(self, channel: GuildChannel) -> None:
        """
        ...
        # Get the audit log entry for this channel creation
    ...
    async def log_channel_delete(self, channel: GuildChannel) -> None:
        """
        ...
        # Get the audit log entry for this channel deletion
        async for entry in channel.guild.audit_logs(
    ...
    async def log_message_delete(self, message: Message) -> None:
        ...
        embed = Embed(
            ...
            description=(
                ...
                f"it was sent on {format_dt(message.created_at, 'D')} ({format_dt(message.created_at, 'R')})"
            ),
        )
        if message.system_content:
            embed.add_field(
    ...
    async def log_role_deletion(self, entry: AuditLogEntry) -> None:
        ...
        embed = Embed(
            title="Role Deleted",
            description=(
                f"Role **{entry.target.mention}** deleted by {entry.user.mention}"
                if entry.user
                else f"Role **{entry.target.mention}** deleted"
                + (" (integration)" if entry.target.is_integration() else "")
            ),
            color=(
                entry.target.color
    ...
    async def log_role_create(self, role: Role) -> None:
        """
        Log when a role is created.
        """
        # Get the audit log entry for this role creation
    ...
    async def log_role_update(self, before: Role, after: Role) -> None:
        """
        Log when a role is updated.
        """
        # Get the audit log entry for this role update
        ...
        # Only handle name changes for now to match the screenshot
        if before.name == after.name:
            return

        embed = Embed(title="Role Updated")
        embed.description = f"The name of this role was changed by {moderator.mention if moderator else 'Unknown'}."
...
#    async def log_channel_permissions_update(self, before: GuildChannel, after: GuildChannel) -> None:
#        """
#        Log when channel permissions are updated.
#        """
#        if isinstance(
#            after,
#            (
...
#            return
#
#        # Get the audit log entry for this permission update
#        async for entry in after.guild.audit_logs(limit=1):
#            if (entry.action in [AuditLogAction.overwrite_update, AuditLogAction.overwrite_create, AuditLogAction.overwrite_delete]
#                and entry.target.id == after.id):
#                moderator = entry.user
#                target = entry.extra  # This is the role or user whose permissions were changed
#
#                # Create the embed
#                embed = Embed(title="Channel Permissions Updated")
#
#                if isinstance(target, Role):
#                    embed.description = f"The overwrites for the {target.mention} role were changed in the {after.mention} channel by {moderator.mention}."
#
#                    # Compare before and after permissions
#                    if entry.before and entry.after:
#                        # Convert to PermissionOverwrite objects
#                        before_perms = entry.before
#                        after_perms = entry.after
#
#                        # Check view channel permission
#                        if getattr(before_perms, "view_channel", None) != getattr(after_perms, "view_channel", None):
#                            value = getattr(after_perms, "view_channel", False)
#                            embed.add_field(
#                                name="View Channels",
#                                value="✅" if value else "❌",
#                                inline=True
#                            )
#
#                        # Check reaction permission
#                        if getattr(before_perms, "add_reactions", None) != getattr(after_perms, "add_reactions", None):
#                            value = getattr(after_perms, "add_reactions", False)
#                            embed.add_field(
#                                name="Add Reactions",
#                                value="✅" if value else "❌",
#                                inline=True
#                            )
#
#                        # Check external emoji permission
#                        if getattr(before_perms, "use_external_emojis", None) != getattr(after_perms, "use_external_emojis", None):
#                            value = getattr(after_perms, "use_external_emojis", False)
#                            embed.add_field(
...
#
#                    if embed.fields:  # Only send if permissions were changed
#                        await self.log(
#                            after.guild,
#                            LogType.CHANNEL,
#                            embed,
...
#                            user=moderator,
#                        )
#                    return  # Exit after logging the audit log entry
#
#        # If we get here, check for bot-initiated changes by comparing overwrites directly
#        if before.overwrites != after.overwrites:
#            for role, after_overwrite in after.overwrites.items():
#                before_overwrite = before.overwrites.get(role)
#                if before_overwrite != after_overwrite:
#                    embed = Embed(title="Channel Permissions Updated")
#                    embed.description = f"The overwrites for the {role.mention} role were created in the {after.mention} channel by {self.bot.user.mention}."
#
#                    # Get the permission pairs for the role
#                    allows, denies = after_overwrite.pair()
#
#                    # Add fields for specific permissions
#                    if denies.add_reactions:
#                        embed.add_field(
#                            name="Add Reactions",
#                            value="❌",
#                            inline=True
#                        )
#                    if denies.use_external_emojis:
#                        embed.add_field(
#                            name="Use External Emojis",
#                            value="❌",
#                            inline=True
#                        )
#                    if allows.view_channel:
#                        embed.add_field(
#                            name="View Channels",
#                            value="✅",
#                            inline=True
