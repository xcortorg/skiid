...
from system.tools.utils import codeblock, vowel, Plural
from system.base.embed import EmbedScript as Script
from system import Marly


class Flags(CustomFlagConverter):
...
class Welcome(MixinMeta, metaclass=CompositeMetaClass):
    """
    ...
    async def notify_failure(
        ...
        if len(script.script) <= 1024:
            embed.add_field(
                name="**Script**",
                value=codeblock(script.script),
            )
        ...
        with suppress(HTTPException):
            return await owner.send(
                embed=embed,
                content=(
                    codeblock(script.script) if len(script.script) > 1024 else None
                ),
            )
    ...
    async def welcome_send(self, member: Member) -> List[Message]:
        ...
        for record in records:
            ...
            script = Script(record["template"])
            try:
                message = await script.send(
                    channel, guild=guild, user=member, channel=channel
                )
    ...
    async def goodbye_send(self, member: Member) -> List[Message]:
        ...
        for record in records:
            ...
            # Fixed: Create script with template and pass context variables as kwargs
            script = Script(record["template"])
            try:
                message = await script.send(
                    channel, guild=guild, user=member, channel=channel
    ...
    async def welcome_add(
        ...
        *,
        ...
        return await ctx.approve(
            f"Added greet message to {channel.mention}",  # Removed vowel(script.format)
            *(
                [
    ...
    async def welcome_list(self, ctx: Context) -> Message:
        ...
        return await ctx.autopaginator(embed=embed, description=channels, split=2)
    ...
    async def goodbye_add(
        ...
        *,
        ...
        # Fix: Use script.script instead of script.template
        template, flags = await Flags().find(ctx, script.script)
        if not template:
