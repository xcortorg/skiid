...
class VoiceMaster(commands.Cog):
    ...
    async def get_owner(self, channel_id):
        row = await self.client.pool.fetchrow(
            "SELECT owner_id FROM voicemaster_owners WHERE channel_id = $1", channel_id
        )
        ...
        if before.channel and before.channel != channel and not before.channel.members:
            ...
            if owner_id == member.id:
                if before.channel.members:
                    ...
                    await self.client.pool.execute(
                        "DELETE FROM voicemaster_owners WHERE channel_id = $1",
                        before.channel.id,
                    )
            ...
            if (
                ...
                if owner_id != interaction.user.id and custom_id != "delete":
                    await interaction.response.send_message(
                        embed=discord.Embed(
                            description=f"> {emoji.deny} {interaction.user.mention}: You are **not** the owner of this voice channel",
                            color=color.deny,
                        ),
                        ephemeral=True,
                    )
                ...
                await interaction.response.send_message(
                    embed=discord.Embed(
                        description=f"> {emoji.deny} {interaction.user.mention}: You are **not** in a voice channel",
                        color=color.deny,
                    ),
                    ephemeral=True,
                )
