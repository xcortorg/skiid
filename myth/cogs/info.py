...
from discord import ButtonStyle, __version__
...
class Information(commands.Cog):
    ...
    @commands.command(description="Check the bot's latency", aliases=["p", "latency"])
    async def ping(self, ctx):
        ...
        embed = discord.Embed(
            description=f"> :mag: Latency: **{latency}ms**", color=color.default
        )
        embed.set_author(name=ctx.author.name, icon_url=user_pfp)
        await ctx.send(embed=embed)

    @commands.command(
        description="Add me im cool :sunglasses:", aliases=["invite", "links"]
    )
    async def inv(self, ctx):
        ...
        inv = Button(
            style=discord.ButtonStyle.link,
            label="Invite me",
            url="https://discordapp.com/oauth2/authorize?client_id=1284613721888526417&scope=bot+applications.commands&permissions=8",
            emoji=f"{emoji.link}",
        )
    ...
    @commands.command(description="Check the bots infos=", aliases=["about", "bi"])
    async def botinfo(self, ctx):
        ...
        inv = Button(
            style=ButtonStyle.link,
            label="Invite me",
            url=f"https://discordapp.com/oauth2/authorize?client_id={self.client.user.id}&scope=bot+applications.commands&permissions=8",
            emoji=emoji.link,
        )

        embed = discord.Embed(
            title="Information",
            description="> Developed by [lavalink](https://github.com/lavalink-dev) & contributed by [misimpression](https://github.com/misimpression)",
            color=color.default,
        )
        embed.add_field(
            name="Statistics",
    ...
    async def userinfo(self, ctx, member: discord.Member = None):
        ...
        if (
            member.guild.premium_subscriber_role
            and member.guild.premium_subscriber_role in member.roles
        ):
            badges.append("<a:boost:1291122311944081531>")

        embed.add_field(
            name="Joined",
            value=f"> {format_dt(member.joined_at, style='D') if hasattr(member, 'joined_at') else 'n/a'} \n> {format_dt(member.joined_at, style='R') if member.joined_at else 'n/a'}",
            inline=True,
        )
        embed.add_field(
            name="Created",
            value=f"> {format_dt(member.created_at, style='D') if hasattr(member, 'created_at') else 'n/a'} \n> {format_dt(member.created_at, style='R') if hasattr(member, 'created_at') else 'n/a'}",
            inline=True,
        )
        embed.add_field(
            name="Extra",
    ...
    @commands.command(description="Get info on a server", aliases=["si", "server"])
    async def serverinfo(self, ctx: commands.Context):
        ...
        boosters = (
            f"{unique_boosters} booster(s)" if unique_boosters > 0 else "0 booster(s)"
        )
        boost = (
            f"{guild.premium_subscription_count} (lvl {guild.premium_tier})"
        ...
        embed.set_author(
            name=f"{guild.name} ({guild.id}) | serverinfo", icon_url=icon_url
        )
        ...
        embed.add_field(
            name="Counts",
            value=f"> **Emojis:** `{len(guild.emojis)}/{guild.emoji_limit}` \n> **Stickers:** `{stickers}/{guild.sticker_limit}` \n> **Roles:** `{len(guild.roles)}/250`",
            inline=True,
        )
        embed.add_field(
            name=f"Design",
            value=f"> **Avatar:** {'[Here](' + pfp + ')' if pfp else 'n/a'}\n> **Banner:** {'[Here](' + banner + ')' if banner else 'n/a'}\n> **Splash:** {'[Here](' + splash + ')' if splash else 'n/a'}",
            inline=True,
        )
        embed.add_field(
            name=f"Boosts",
            value=f"> **Boosts:** `{boost}`\n> **Boosters:** `{boosters}`\n> **Vanity:** `{vanity}`",
            inline=True,
        )
        await ctx.send(embed=embed)
    ...
    @commands.command(
        description="Check a users banner", aliases=["bnner", "bnr", "bn"]
    )
    ...
    @commands.command(
        description="Check the server avatar", aliases=["sa", "servericon"]
    )
    async def serveravatar(self, ctx: Context):
        if ctx.guild:
            if ctx.guild.icon:
                ...
                embed.set_author(
                    name=f"{ctx.author.name} | server avatar", icon_url=user_pfp
                )
                view = View()
                view.add_item(
                    Button(
    ...
    async def serverbanner(self, ctx: Context):
        if ctx.guild:
            if ctx.guild.banner:
                ...
                embed.set_author(
                    name=f"{ctx.author.name} | server banner", icon_url=user_pfp
                )
                view = View()
                view.add_item(
                    Button(
    ...
    async def bans(self, ctx: commands.Context):
        ...
        banned_list = [
            f"> {ban_entry.user.mention} - {ban_entry.reason or 'No reason'}"
        ...
        pages = [
            discord.Embed(
                description="\n".join(banned_list[i : i + 10]), color=color.default
            ).set_author(name=f"{ctx.author.name} | Everyone banned", icon_url=user_pfp)
    ...
    async def boosters(self, ctx: Context):
        ...
        pages = [
            discord.Embed(
                description="\n".join(booster_list[i : i + 10]), color=color.default
            ).set_author(
                name=f"{ctx.author.name} | Everyone who boosted", icon_url=user_pfp
            )
    ...
    async def bots(self, ctx: Context):
        ...
        pages = [
            discord.Embed(
                description="\n".join(bot_list[i : i + 10]), color=color.default
            ).set_author(name=f"{ctx.author.name} | Every bot", icon_url=user_pfp)
    ...
    @commands.command(description="Check all users in a role")
    async def inrole(self, ctx: Context, role: discord.Role):
        ...
        if members_with_role:
            ...
            for i in range(0, len(members_with_role), 10):
                embed = discord.Embed(
                    description="\n".join(members_with_role[i : i + 10]),
                    color=color.default,
                )
                embed.set_author(
                    name=f"{ctx.author.name} | People inrole: {role.name}",
                    icon_url=user_pfp,
                )
                pages.append(embed)
    ...
    async def joinposition(self, ctx: Context):
        join_position = (
            sorted(ctx.guild.members, key=lambda m: m.joined_at).index(ctx.author) + 1
        )
        await ctx.invisible(
            f"You joined **{join_position}** that's a really cool position :sunglasses:"
        )
    ...
    async def roleinfo(self, ctx, *, role: discord.Role):
        ...
        embed.set_author(
            name=f"{ctx.author.name} | roleinfo - {name}", icon_url=user_pfp
        )

        embed.add_field(
            name="General",
            value=f"> **ID:** `{id}` \n> **Color:** `{color}` \n> **Position:** `{position}/{len(guild.roles)}`",
            inline=True,
        )
        embed.add_field(name="Created", value=f"> {created}", inline=False)
        embed.add_field(
            name="Members with the role", value=f"> {displayed}", inline=True
        )
    ...
    async def channelinfo(self, ctx, channel: discord.TextChannel = None):
        ...
        embed.set_author(
            name=f"{ctx.author.name} | channelinfo - {name}", icon_url=user_pfp
        )
    ...
    async def youngest(self, ctx):
        youngest = sorted(ctx.guild.members, key=lambda m: m.created_at, reverse=True)[
            0
        ]
        account_age = format_dt(youngest.created_at, "F")
        account_age2 = format_dt(youngest.created_at, "R")
        await ctx.invisible(
            f"The youngest user in the server is {youngest.mention}, who created their account {account_age2} ({account_age})"
        )
    ...
    async def oldest(self, ctx):
        ...
        await ctx.invisible(
            f"The oldest member in the server is {oldest.mention}, who created their account {account_age2} ({account_age})"
        )
    ...
    async def shards(self, ctx):
        embed = discord.Embed(
            description=f"> Shard count: `{self.client.shard_count}`",
            color=color.default,
        )
        user_pfp = (
            ctx.author.avatar.url
        ...
        for shard_id in range(self.client.shard_count):
            ...
            users_shard = sum(
                g.member_count for g in self.client.guilds if g.shard_id == shard_id
            )
            ...
            embed.add_field(
                ...
                value=f"> Guilds: `{guilds_shard}`\n> Users: `{users_shard:,}`",
                inline=True,
            )

        await ctx.send(embed=embed)

    @commands.command(description="Check the total member count", aliases=["mc"])
