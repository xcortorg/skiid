...
class Economy(commands.Cog):
    ...
    async def transfer(
        ...
        *,
        ...
        if member.bot:
            return await ctx.warn("You cannot transfer credits to a bot")
    ...
    async def deposit(self: "Economy", ctx: Context, amount: Annotated[int, Value]):
        ...
        return await ctx.approve(f"Transfered `{amount:,}` credits to bank")
    ...
    async def withdraw(self: "Economy", ctx: Context, amount: Annotated[int, Bank]):
        ...
        return await ctx.approve(f"Withdrawn `{amount:,}` credits from the bank")
    ...
    async def work_invoke(self, _):
        if not self.jobs:
            async with self.bot.session.get(
                "https://www.randomlists.com/data/jobs.json"
            ) as response:
                data = await response.json()  # Parse the response as JSON
                self.jobs = data["data"]  # Access the 'data' key
    ...
    async def rob(self: "Economy", ctx: Context, *, member: discord.Member):
        ...
        if member == ctx.author:
            return await ctx.warn("You cannot rob yourself")

        if member.bot:
            return await ctx.warn("You cannot rob a bot")
        ...
        if not member_credits or member_credits == 0:
            return await ctx.warn("This member has no money")
    ...
    async def leaderboard(self: "Economy", ctx: Context):
        ...
        if not results:
            return await ctx.warn("There are no members to display on the leaderboard")

        description = [
            f"`{index + 1:02}` {self.bot.get_user(r.user_id)} (`{r.user_id}`) has **{(r.credits + r.bank):,}** total credits"
            for index, r in enumerate(results)
        ]

        embed = discord.Embed(title=f"Economy leaderboard ({len(results)})")

        return await ctx.autopaginator(embed=embed, description=description, split=10)
    ...
    async def balance(
        ...
        *,
        ...
        if not result:
            return await ctx.warn("This member doesn't have an account open")
    ...
    @discord.app_commands.allowed_installs(guilds=True, users=True)
    @discord.app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def openaccount(self: "Economy", ctx: Context):
        """
        Open an account in the economy system
        """
        # Check if the user already has an account
        existing_account = await self.bot.db.fetchval(
            "SELECT user_id FROM economy WHERE user_id = $1", ctx.author.id
        )

        if existing_account:
            return await ctx.warn("You already have an account open.")

        # Insert a new account for the user
        await self.bot.db.execute(
            """
            INSERT INTO economy (user_id, credits, bank, daily, monthly)
            VALUES ($1, $2, $3, $4, $5)
            """,
            ctx.author.id,
            0,  # Initial credits
            0,  # Initial bank balance
            discord.utils.utcnow() + datetime.timedelta(days=1),  # Daily reset
            discord.utils.utcnow() + datetime.timedelta(days=30),  # Monthly reset
        )

        return await ctx.approve("Your account has been successfully opened!")

    @commands.hybrid_command()
    @discord.app_commands.allowed_installs(guilds=True, users=True)
    @discord.app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def closeaccount(self: "Economy", ctx: Context):
        """
        Close your account in the economy system
        """
        ...
        if not existing_account:
            return await ctx.warn("You do not have an account to close.")

        # Delete the user's account
        await self.bot.db.execute(
            "DELETE FROM economy WHERE user_id = $1", ctx.author.id
        )

        return await ctx.approve("Your account has been successfully closed.")
