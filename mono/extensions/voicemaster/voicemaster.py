...
class VoiceMaster(Cog):
    ...
    async def voicemaster_setup(self, ctx: Context):
        ...
        embed.add_field(
            name="**Button Usage**",
            value=(
                f"{config.Emojis.Interface.lock} — [`Lock`]({config.Mono.support}) the voice channel\n"
                f"{config.Emojis.Interface.unlock} — [`Unlock`]({config.Mono.support}) the voice channel\n"
                f"{config.Emojis.Interface.ghost} — [`Ghost`]({config.Mono.support}) the voice channel\n"
                f"{config.Emojis.Interface.reveal} — [`Reveal`]({config.Mono.support}) the voice channel\n"
                f"{config.Emojis.Interface.claim} — [`Claim`]({config.Mono.support}) the voice channel\n"
                f"{config.Emojis.Interface.disconnect} — [`Disconnect`]({config.Mono.support}) a member\n"
                f"{config.Emojis.Interface.activity} — [`Start`]({config.Mono.support}) a new voice channel activity\n"
                f"{config.Emojis.Interface.information} — [`View`]({config.Mono.support}) channel information\n"
                f"{config.Emojis.Interface.increase} — [`Increase`]({config.Mono.support}) the user limit\n"
                f"{config.Emojis.Interface.decrease} — [`Decrease`]({config.Mono.support}) the user limit\n"
            ),
        )
