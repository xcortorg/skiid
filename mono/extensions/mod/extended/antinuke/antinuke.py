...
class Settings(BaseModel):
    ...
    async def check_threshold(self, bot: Mono, member: Member, module: str) -> bool:
        """
        ...
        if value >= config.threshold:
            log.debug(
                f"{member} exceeded the antinuke {module} threshold in {self.guild}."
            )
            return True
    ...
    @classmethod
    async def revalidate(cls, bot: Mono, guild: Guild) -> Optional[Self]:
        """
    ...
    @classmethod
    async def fetch(cls, bot: Mono, guild: Guild) -> Self:
        """
...
class AntiNuke(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def antinuke_whitelist_list(self, ctx: Context) -> Message:
        ...
        return await ctx.autopaginator(embed=embed, description=members, split=10)
    ...
    async def antinuke_kick(
        ...
        *,
        ...
        # Directly update the cache with the new settings
        key = xxh32_hexdigest(f"antinuke:{ctx.guild.id}")
        await self.bot.redis.set(key, config.dict(exclude={"guild"}))

        if not status:
            return await ctx.approve("No longer preventing members from being kicked")
    ...
    @antinuke.command(name="reset", brief="antinuke owner & admin")
