...
class ReactionRoles(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def reactionrole_list(self, ctx: Context) -> Message:
        ...
        messages = [
            f"`{index:02}` [`{message.id}`]({message.jump_url}) - {role.mention} for **{record['emoji']}**"
        ...
        embed.set_footer(text=f"Total reaction roles: {len(messages)}")
        return await ctx.autopaginator(embed, messages, split=10)
