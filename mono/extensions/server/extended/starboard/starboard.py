...
class StarboardConfig:
    ...
    def __init__(self, *, bot: Mono, record: StarboardRecord):
        self.bot = bot
...
class Starboard(Cog):
    ...
    async def starboard_list(self, ctx: Context) -> Message:
        ...
        channels = [
            f"`{index:02}` {channel.mention} - **{record['emoji']}** (threshold: `{record['threshold']}`, author: `{record['self_star']}`)"
        ...
        embed.set_footer(text=f"Total starboards: {len(channels)}")
        return await ctx.autopaginator(embed, channels)
