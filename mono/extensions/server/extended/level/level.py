...
class Level(MixinMeta, metaclass=CompositeMetaClass):
        ...
        *,
        ...
        if not record:
            return await ctx.warn(
                f"No level data has been recorded yet!\nMake sure the level system is enabled by using `{ctx.clean_prefix}level toggle`"
            )
    ...
    async def level_leaderboard(self, ctx: Context) -> Message:
        ...
        if not members:
            return await ctx.warn(
                "No level data has been recorded yet!\nMake sure the level system is enabled by using `{ctx.clean_prefix}level toggle`"
            )
    ...
    async def level_role_list(self, ctx: Context) -> Message:
        ...
        return await ctx.autopaginator(embed=embed, description=fields, split=6)
