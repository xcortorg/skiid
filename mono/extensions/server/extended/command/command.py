...
class CommandManagement(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def check_command_restrictions(self, ctx: Context) -> bool:
        ...
        # Allow commands in DM channels
        if not ctx.guild:
            return True  # Allow all commands in DMs

        if not ctx.command or not isinstance(ctx.author, Member):
            return True  # Allow if no command or author is not a Member

        # Check if ctx.author is None
        if ctx.author is None:
            return False  # Deny if author is None

        if ctx.author.guild_permissions.administrator:
            return True  # Allow if the user is an administrator

        # Ensure ctx.author is a Member and has roles
        if not isinstance(ctx.author, Member) or not ctx.author.roles:
            return False
            ...
            """
                ...
                """,
            ctx.guild.id,
            [role.id for role in ctx.author.roles if role] if ctx.author.roles else [],
            ctx.command.qualified_name,
            (
                ctx.command.parent.qualified_name  # type: ignore
                if ctx.command.parent
    ...
    async def ignore_list(self, ctx: Context) -> Message:
        ...
        return await ctx.autopaginator(embed=embed, description=targets, split=10)
    ...
    async def command_disable_list(self, ctx: Context) -> Message:
        ...
        embed = Embed(title="Commands Disabled")
        return await ctx.autopaginator(embed=embed, description=commands, split=10)
    ...
    async def command_restrict_list(self, ctx: Context) -> Message:
        ...
        embed = Embed(title="Command Restrictions")
        return await ctx.autopaginator(embed=embed, description=commands, split=10)
