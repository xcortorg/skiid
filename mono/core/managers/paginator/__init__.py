...
class Paginator(discord.ui.View):
    def __init__(
        ...
        *,
        ...
        if len(self.embeds) > 1:
            ...
            # Add the random page button
            self.random_page_button = discord.ui.Button(
                style=discord.ButtonStyle.blurple, emoji=config.Emojis.Paginator.random
            )
            self.random_page_button.callback = self.random_page
            self.add_item(self.random_page_button)
    ...
    async def random_page(self, interaction: discord.Interaction):
        if len(self.embeds) <= 2:
            # If there are only 2 pages or fewer, just go to the other page
            new_page = (self.page + 1) % len(self.embeds)
        else:
            # Generate a list of valid page numbers, excluding the current page
            valid_pages = [i for i in range(len(self.embeds)) if i != self.page]
            new_page = random.choice(valid_pages)

        await self.change_page(interaction, new_page)
