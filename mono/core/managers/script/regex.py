...
TIKTOK_DESKTOP_URL = re.compile(
    r"(?:http\:|https\:)?\/\/(?:www\.)?tiktok\.com\/@.*\/video\/\d+"
)
TIKTOK_MOBILE_URL = re.compile(
...
YOUTUBE_SHORT_URL = re.compile(
    r"(?:http\:|https\:)?\/\/(?:www\.)?youtu\.be\/(?P<id>[a-zA-Z0-9_-]+)"
)
YOUTUBE_SHORTS_URL = re.compile(
    r"(?:http\:|https\:)?\/\/(?:www\.)?youtube\.com\/shorts\/(?P<id>[a-zA-Z0-9_-]+)"
)
YOUTUBE_CLIP_URL = re.compile(
    r"(?:http\:|https\:)?\/\/(?:www\.)?youtube\.com\/clip\/Ug(?P<id>[a-zA-Z0-9_-]+)"
)
YOUTUBE_CHANNEL_URL = re.compile(
    r"(?:http\:|https\:)?\/\/(?:www\.)?youtube\.com\/(?P<type>channel/|user/|c/|@)(?P<id>[a-zA-Z0-9_-]+)"
)
...
PINTEREST_PIN_URL = re.compile(
    r"(?:http\:|https\:)?\/\/(?:www\.)?pinterest\.com\/pin\/(?P<id>[0-9]+)"
)
PINTEREST_PIN_APP_URL = re.compile(
    r"(?:http\:|https\:)?\/\/pin\.it\/(?P<id>[a-zA-Z0-9]+)"
)
GRAILED_LISTING_URL = re.compile(
    r"(?:http\:|https\:)?\/\/(?:www\.)?grailed\.com\/listings\/(?P<id>[0-9]+)"
)
GRAILED_LISTING_APP_URL = re.compile(
    r"(?:http\:|https\:)?\/\/(?:www\.)?grailed\.app\.link\/(?P<id>[a-zA-Z0-9]+)"
)
