...
class OpenTicket(
    ...
    async def create_channel(
        ...
        # Check for topic-specific category
        if topic:
        ...
        # Set default overwrites to hide from everyone
        overwrites = {}
        ...
        # Add ticket creator permissions
        ...
        # Add topic-specific role permissions - these are the only roles that can see this ticket type
        if topic:
            ...
            # If no roles are set for this topic, allow the default support role to see it
        ...
        channel = await interaction.guild.create_text_channel(
            ...
            category=category,
            topic=f"A ticket opened by {interaction.user.name} ({interaction.user.id})" + (f" | Topic: {topic}" if topic else ""),
        ...
        # Store the topic for this ticket if provided
        if topic:
            await interaction.client.db.execute(
    ...
    async def callback(self, interaction: Interaction) -> None:
        ...
        category = interaction.guild.get_channel(check["category_id"]) if check["category_id"] else None
    ...
    async def callback(self, interaction: Interaction) -> None:
        ...
        async def yes_callback(inter: Interaction) -> None:
            ...
            try:
                ...
                await inter.channel.delete(reason=f"Ticket closed by {interaction.user}")
...
class Tickets(Cog):
    ...
    async def ticket_support(self, ctx: Context, *, role: Role = None):
        ...
        if role:
            ...
            # Get topics without specific roles
            ...
            topic_list = "\n".join([f"• {topic['name']}" for topic in topics_without_roles]) if topics_without_roles else "None"
    ...
    async def ticket_topics(self, ctx: Context):
        ...
        async def button2_callback(interaction: Interaction):
            ...
            async def select_callback(inter: Interaction):
                ...
                # Delete the topic and all associated data
                await self.bot.db.execute(
        ...
        async def button4_callback(interaction: Interaction):
            e = Embed(color=self.bot.color, description="🔍 Select a topic to manage")
            options = [
                SelectOption(label=result[1], description=result[2])
                for result in results
            ]
            ...
            async def select_callback(inter: Interaction):
                ...
                # Get current roles and category for this topic
                ...
                remove_role = Button(label="Remove Role", style=ButtonStyle.red, disabled=len(role_ids) == 0)
                ...
                async def remove_role_callback(interaction: Interaction):
                    ...
                    async def role_select_callback(select_inter: Interaction):
                        ...
                        role_name = role.name if role else f"Role (ID: {selected_role_id})"
    ...
    async def ticket_setup(
        ...
        self.bot.cw = ctx
        code = ctx.parameters.get("code") or ctx.parameters.get("embed") or "{embed}{color: #2b2d31}$v{title: Create a ticket}$v{description: Click on the button below this message to create a ticket}"
    ...
    async def ticket_send(
        ...
        if not check:
            return await ctx.fail("Please set up tickets first using the setup command")
        
        view = TicketView(self.bot, ctx.guild.id)
        await view.setup()
        view.create_ticket()
        
        embed_code = check.get("open_embed") or "{embed}{color: #2b2d31}$v{title: Create a ticket}$v{description: Click on the button below this message to create a ticket}"
        
        message = await self.bot.send_embed(channel, embed_code, user=ctx.author, view=view)
        await self.bot.db.execute(
            """UPDATE tickets SET message_id = $1 WHERE guild_id = $2""",
            message.id,
            ctx.guild.id,
...
class RoleInputModal(Modal):
    ...
    async def on_submit(self, interaction: Interaction):
        try:
            ...
            if existing:
                return await interaction.response.send_message(
                    f"Role {role.mention} is already assigned to this topic.", ephemeral=True
                )
...
class CategoryInputModal(Modal):
    ...
    async def on_submit(self, interaction: Interaction):
        try:
            ...
            # Check if category has proper permissions
            ...
            await interaction.response.send_message(
                "Please enter a valid category ID (numbers only).", 
                ephemeral=True
            )
        except Exception as e:
            await interaction.response.send_message(
