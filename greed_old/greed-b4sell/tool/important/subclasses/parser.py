...
class Script:
    def __init__(
        ...
        self.data: Dict[str, Union[Dict, str, List]] = {
            "embed": {},
            "buttons": [],
        }
        self.replacements = {
    ...
    def parse_button(self, value: str) -> None:
        ...
        try:
            ...
            if "buttons" not in self.data:
                self.data["buttons"] = []
                
            self.data["buttons"].append({"label": label, "url": url})
    ...
    async def compile(self: Self) -> None:
        ...
        try:
            ...
            if "button" in self.data and not self.data["buttons"]:
                self.data["buttons"].append(self.data.pop("button"))
    ...
    async def send(
        ...
        try:
            if buttons := self.data.pop("buttons", None):
                view = View()
