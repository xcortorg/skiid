...
class TaskNode:
    ...
    def __post_init__(self):
        # Validate that the function is not already a result of computation
...
class LazyResult:
    ...
    async def compute(self) -> Any:
        ...
        # Check for cached result if pure function
        if self.task.pure:
        ...
        # Replace LazyResult arguments with computed values
        args = []
        for arg in self.task.args:
            if isinstance(arg, LazyResult):
                args.append(await arg.compute())
            else:
                # Make a copy to prevent mutations
