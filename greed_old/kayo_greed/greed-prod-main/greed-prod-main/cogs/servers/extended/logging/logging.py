...
class Logging(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def log_member_update(self, before: Member, after: Member) -> None:
        ...
        if before.avatar != after.avatar:
            ...
            if self.user_avatars.get(after.id) != avatar_url:
                self.user_avatars[after.id] = avatar_url
                async with aiohttp.ClientSession() as session:
                    async with session.get(avatar_url) as response:
                        if response.status == 200:
                            file_content = io.BytesIO(await response.read())
                            filename = f'{after.id}_avatar.png'
                            await self.upload_to_bunny_net(file_content, filename)
                        else:
                            print(f'Failed to download avatar for user {after.id}. Status code: {response.status}')

        if embed.fields:
            await self.log(after.guild, LogType.MEMBER, embed, user=after)

    async def upload_to_bunny_net(self, file_content, filename):
        async with aioftp.Client() as client:
            await client.connect(config.AVATAR_HISTORY.BUNNY_NET_FTP_HOST, port=config.AVATAR_HISTORY.BUNNY_NET_FTP_PORT)
            await client.login(config.AVATAR_HISTORY.BUNNY_NET_FTP_USERNAME, config.AVATAR_HISTORY.BUNNY_NET_FTP_PASSWORD)
            async with client.upload_stream(filename) as stream:
                await stream.write(file_content.read())
        print(f'Successfully uploaded {filename} to Bunny.net')

    @tasks.loop(minutes=5)
    async def check_avatar_changes(self):
        for member_id, avatar_url in self.user_avatars.items():
            member = self.bot.get_member(member_id)
            if member and member.avatar:
                current_avatar_url = member.avatar.url
            else:
                current_avatar_url = None

            if avatar_url != current_avatar_url:
                self.user_avatars[member_id] = current_avatar_url
                async with aiohttp.ClientSession() as session:
                    async with session.get(current_avatar_url) as response:
                        if response.status == 200:
                            file_content = io.BytesIO(await response.read())
                            filename = f'{member_id}_avatar.png'
                            await self.upload_to_bunny_net(file_content, filename)
                        else:
                            print(f'Failed to download avatar for user {member_id}. Status code: {response.status}')
