...
from aiohttp import ClientSession as OriginalClientSession, ClientTimeout, TCPConnector, ClientResponseError, ClientConnectionError, ClientError
        ...
        while attempt < self.retry_attempts:
            try:
                ...
                if response.status == 429:
                    ...
                    log.warning(f"Last.fm API rate limit exceeded, retrying after {delay} seconds...")
                ...
                log.error(f"Response error occurred: {e.status} - {e.message} from {e.request_info.url}")
                attempt += 1
                ...
                raise
        log.error(f"Failed to get a successful response after {self.retry_attempts} attempts")
