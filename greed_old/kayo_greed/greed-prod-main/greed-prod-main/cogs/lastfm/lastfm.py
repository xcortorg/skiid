...
from discord import app_commands, Color, Embed, File, HTTPException, Member, Message, Interaction
...
class Lastfm(Cog):
    ...
    @app_commands.describe(member="The member whose current playing status you want to check")
    @app_commands.allowed_installs(guilds=True, users=True)
    @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def nowplaying_slash(self, interaction: Interaction, member: Member = None):
        """
        Invoke the nowplaying text command
        """
        ctx = await DefaultContext.from_interaction(interaction)
        member = member or interaction.user
        

        await self.nowplaying(ctx, member=member)

    @app_commands.command(name="spotify")
    @app_commands.describe(query="The name of the song, defaults to currently playing")
    @app_commands.allowed_installs(guilds=True, users=True)
    @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def spotify_slash(self, interaction: Interaction, query: Optional[str] = None):
        """
        ...
        await self.spotify(ctx, query=query)


    @command(aliases=["now", "np", "fm"], usage="[member]", brief="@66adam")
    ...
    async def lastfm_set(
        ...
        """
        Connect your Last.fm account to greed's Last.FM system.
        """
    ...
    async def lastfm_mode(
        ...
        *,
        ...
        Available variables can be found via our [documentation](https://docs.greed.bot/intro/#last-fm).
        """
    ...
    async def lastfm_mode_remove(self, ctx: Context) -> Message:
        """
        Remove your current custom embed mode from greed's database.
        """
    ...
    async def lastfm_reactions_remove(self, ctx: Context) -> Message:
        """
        Remove your custom reactions from greed's database.
        """
    ...
    @lastfm.command(name="crowns", usage="<member>", brief="@66adam")
    ...
    async def lastfm_hide_list(self, ctx: Context) -> Message:
        """
        View all hidden members from greed's LastFM.
        """
