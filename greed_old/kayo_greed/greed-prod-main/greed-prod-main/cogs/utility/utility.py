...
class Utility(Extended, Cog):
    ...
    async def google(self, ctx: Context, *, query: str) -> Message:
        ...
        for i in range(0, total_results, per_page):
            ...
            for result in page_embeds:
                embed.add_field(name=f"**{result.title}**", value=field_value, inline=False)
    ...
    @command(aliases=["parse", "ce"], description="manage messages", brief="{title: Hello World!}")
    ...
    async def recolor(self, ctx: Context, hex_color: str, image_url: Optional[str] = None):
                ...
                async with self.bot.session.get(target) as response:
                    if response.status == 200:
                        ...
                        if len(data) > MAX_BYTES:
                            return await ctx.warn("The response is too large to handle.")
                    else:
                        await ctx.warn("Could not fetch the image from the provided URL.")
