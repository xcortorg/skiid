...
from typing import TYPE_CHECKING, AsyncGenerator, Generator, List, Optional, Tuple, Any, Callable, Dict
...
def unicode_emoji(emoji: str) -> tuple[str, str]:
    ...
    if "20e3" in characters:
        characters.remove("fe0f")
    return (
        "https://cdn.jsdelivr.net/gh/jdecked/twemoji@latest/assets/svg/" + "-".join(characters) + ".svg",
        "_".join(name),
    )
...
def enlarge_emoji(buffer: bytes, format: str) -> Tuple[Optional[bytes], str]:
    ...
    with Image(blob=buffer) as image:
        ...
        image.resize(width=image.width * factor, height=image.height * factor, filter="lanczos2sharp")
        return image.make_blob(), format
...
def capture_time(msg: Optional[str] = None, log: Optional[Logger] = None) -> Generator:
    ...
    try:
        ...
        duration = time() - start
        log.info(f"{msg} {Fore.LIGHTWHITE_EX}{Style.DIM}{fmtseconds(duration)}{Fore.RESET}{Style.NORMAL}.")

def is_dangerous(role: Role) -> bool:
    return any(
        value and permission in (
            "administrator",
            "kick_members",
            "ban_members",
            "manage_guild",
...
async def strip_roles(member: Member, *, dangerous: bool = False, reason: Optional[str] = None) -> bool:
    bot = member.guild.me
...
def uwuify(text: str) -> str:
    ...
    text = " ".join([word if i % 5 else word + ' ' + faces[i % len(faces)] for i, word in enumerate(text_split)])
