...
class Paginator(View):
    ...
    def add_buttons(self):
        self.add_item(Button(custom_id="previous", style=ButtonStyle.secondary, emoji=EMOJIS.PAGINATOR.PREVIOUS or "⬅"))
        self.add_item(Button(custom_id="navigation", style=ButtonStyle.primary, emoji=EMOJIS.PAGINATOR.NAVIGATE or "🔢"))
        self.add_item(Button(custom_id="next", style=ButtonStyle.secondary, emoji=EMOJIS.PAGINATOR.NEXT or "➡"))
        self.add_item(Button(custom_id="cancel", style=ButtonStyle.primary, emoji=EMOJIS.PAGINATOR.CANCEL or "⏹"))
    ...
    def prepare_entries(
        ...
        if isinstance(entries[0], Embed):
            for index, entry in enumerate(entries):
                ...
                entry.set_footer(text=f"Page {index + 1} of {pages}")
            ...
            for chunk in as_chunks(entries, per_page):
                ...
                for value in chunk:
                    offset += 1
                    entry.description += f"`{offset}` {value}\n" if counter else f"{value}\n"
                if pages > 1:
                    footer = entry.footer
            ...
            for index, entry in enumerate(entries):
                compiled.append(f"({index + 1}/{len(entries)}) {entry}" if counter else entry)
    ...
    async def start(self) -> Message:
        ...
        if len(self.entries) == 1:
            self.message = await self.ctx.send(content=page) if isinstance(page, str) else await self.ctx.send(embed=page)
        else:
            self.message = await self.ctx.send(content=page, view=self) if isinstance(page, str) else await self.ctx.send(embed=page, view=self)
    ...
    async def callback(self, interaction: Interaction, button: Button):
        ...
        if button.custom_id == "previous":
            ...
            try:
                response = await self.ctx.bot.wait_for(
                    "message",
                    timeout=60,
                    check=lambda m: m.author == interaction.user and m.channel == interaction.channel and m.content.isdigit() and 1 <= int(m.content) <= len(self.entries),
                )
        ...
        page = self.entries[self.index]
        with suppress(HTTPException):
            await self.message.edit(content=page if isinstance(page, str) else None, embed=page if isinstance(page, Embed) else None, view=self)