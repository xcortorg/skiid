...
from typing import Any, Callable, Coroutine, Generic, MutableMapping, Optional, Protocol, TypeVar
...
class ExpiringCache(dict, Generic[R]):
    ...
    def __getitem__(self, key: Optional[str]) -> tuple[R, float]:
        self.__verify_cache_integrity()
        return super().__getitem__(key)

    def __setitem__(self, key: Optional[str], value: R) -> None:
...
def cache(
    ...
    def decorator(func: Callable[..., Coroutine[Any, Any, R]]) -> CacheProtocol[R]:
        ...
        def _make_key(args: tuple[Any, ...], kwargs: dict[str, Any]) -> str:
            ...
            if not ignore_kwargs:
                for k, v in kwargs.items():
                    if k in ["connection", "pool"]:  # Add more as needed
                        continue
        ...
        def _invalidate_containing(key: str | int) -> None:
            key_str = str(key)
            to_remove = [k for k in _internal_cache.keys() if key_str in k]
            for k in to_remove:
