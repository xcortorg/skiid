...
class greedTraceback(Traceback):
    # DEP-WARN
    @group()
...
class greedLogRender(LogRender):
    def __call__(
        self,
        console: Console,
...
class greedRichHandler(RichHandler):
    ...
    def emit(self, record: LogRecord) -> None:
        """Invoked by logging."""
        path = getattr(record, '_cached_path', None) or pathlib.Path(record.pathname).name
        record._cached_path = path
        ...
        if (
            ...
            traceback = greedTraceback.from_exception(
                exc_type,
                exc_value,
                exc_traceback,
...
def init_logging(level: int) -> None:
    ...
    stdout_handler = greedRichHandler(
