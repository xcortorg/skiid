...
class Snipe(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def snipe(self, ctx: Context, index: int = 1) -> None:
        ...
        if (
            ...
            if message.filtered:
                await ctx.reply("bot filtered the msg lil bro")
                return
            ...
            if config:
                ...
                for word in config["words"]:
                    if word in message.content.lower():
                        await ctx.reply("that msg probably shouldn't be sniped")
                        return
        ...
        if deleted_at is None:
            deleted_at_aware = now_utc 
        elif deleted_at.tzinfo is None:
            deleted_at_aware = deleted_at.replace(tzinfo=timezone.utc)
        else:
        ...
        embed.set_footer(
            text=" • ".join(
                [
                    f"{index}/{snipes}",
                    (
                        f"Deleted {naturaldelta(now_utc - deleted_at_aware)} ago"
                        if deleted_at
                        else "Deleted at an unknown time"
                    ),
                ]
            ),
        )
        ...
        if message.attachments:
            for attachment in message.attachments:
                ...
                if attachment and attachment.size <= 25e6:
            ...
            embed.add_field(
                ...
                value="\n".join([a.url for a in message.attachments if a]),
            )
