...
class MessageSnipe(BaseModel):
    ...
    @classmethod
    async def push(cls, redis: Redis, message: Message) -> Self | None:
        if not message.guild or (message.author.bot and not message.attachments) or message.content.strip() == ".pick":
            return None
    ...
    @classmethod
    async def get(cls, redis: Redis, channel_id: int, index: int = 1) -> Self | None:
