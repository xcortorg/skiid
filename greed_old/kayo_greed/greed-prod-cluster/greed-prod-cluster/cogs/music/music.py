...
from discord.ext.commands import hybrid_command, Cog, hybrid_group, command
import asyncio
...
class Music(Cog):
    ...
    async def now_playing(self, ctx: Context) -> Message:
        ...
        async with self.bot.session.get(url, params=params) as response:
            if response.status != 200:
                return await ctx.warn(
                    f"Failed to fetch now playing cover. {response.status}"
                )
        ...
        async with self.bot.session.get(image_url) as image_response:
            if image_response.status != 200:
                return await ctx.warn(
                    f"Failed to download now playing cover. {image_response.status}"
                )
    ...
    @command(name="lyrics")
    @Donator()
    async def lyrics(self, ctx: Context) -> Message:
        """
        Fetches lyrics for the currently playing song and displays them line by line.
        Stops after LYRICS_DURATION seconds for non-donators.
        """
        try:
            active_session = await self.bot.redis.get(f"active_lyrics:{ctx.guild.id}")
            if active_session:
                return await ctx.send("Lyrics are already running in this server.")
            
            await self.bot.redis.set(f"active_lyrics:{ctx.guild.id}", "active", self.LYRICS_DURATION)

            player: Player = ctx.voice_client
            if player is None or not player.is_playing():
                await self.bot.redis.delete(f"active_lyrics:{ctx.guild.id}")
            ...
            lyrics_url = "https://ws.listenbot.site/lyrics"
            async with self.bot.session.get(lyrics_url, headers=headers, params=payload) as response:
                if response.status != 200:
                    raise Exception(f"Failed to fetch lyrics: {response.status}")
                
                data = await response.json()
                if "result" not in data or not data["result"].get("lyrics"):
                    await self.bot.redis.delete(f"active_lyrics:{ctx.guild.id}")
                    return await ctx.send("Lyrics not found for this song.")
            
            lyrics = data["result"]["lyrics"]
            current_position_ms = player.position * 1000  # Track position in milliseconds

            start_index = next((index for (index, line) in enumerate(lyrics) if line['milliseconds'] >= current_position_ms), None)
            if start_index is None:
                await self.bot.redis.delete(f"active_lyrics:{ctx.guild.id}")
                return await ctx.send("No lyrics found for the current timestamp.")
            
            async def display_lyrics():
                message = await ctx.send("Starting lyrics...")
                try:
                    line_index = start_index
                    elapsed_time = 0

                    while elapsed_time < self.LYRICS_DURATION and line_index < len(lyrics):
                        displayed_lines = ""
                        ...
                        for i in range(10):
                            ...
                            displayed_lines += f"{line['line']}\n"
                            line_index += 1

                            if line_index < len(lyrics):
                                sleep_time = (lyrics[line_index]['milliseconds'] - line['milliseconds']) / 1000.0
                                await asyncio.sleep(sleep_time)

                        await message.edit(content=f"```\n{displayed_lines}```")
                        elapsed_time += sum([lyrics[idx]['milliseconds'] for idx in range(start_index, line_index)]) / 1000.0

                except asyncio.CancelledError:
                    log.warning("Lyrics display task was cancelled.")
                    await ctx.send("Lyrics display cancelled.")
                finally:
                    await self.bot.redis.delete(f"active_lyrics:{ctx.guild.id}")
            
            task = asyncio.create_task(display_lyrics())

            is_donator = await Donator(ctx)
            if not is_donator:
                await asyncio.sleep(self.LYRICS_DURATION)
                task.cancel()
                await ctx.send("Lyrics display has ended.")
        
        except Exception as e:
            log.error(f"An error occurred in the lyrics command: {str(e)}")
            await ctx.send("An error occurred while fetching or displaying the lyrics.")
            await self.bot.redis.delete(f"active_lyrics:{ctx.guild.id}")