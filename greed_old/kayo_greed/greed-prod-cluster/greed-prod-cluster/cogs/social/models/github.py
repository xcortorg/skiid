...
class GitHubProfile(BaseModel):
    ...
    def from_html(cls, html_text: str) -> "GitHubProfile":
        ...
        profile = cls()
        profile.username = tree.xpath(
            '//span[@class="p-nickname vcard-username d-block"]/text()'
        )[0].strip()
        profile.name = tree.xpath(
            '//span[@class="p-name vcard-fullname d-block overflow-hidden"]/text()'
        )[0].strip()
        profile.avatar_url = (
            URL(
                tree.xpath(
        ...
        bio_elements = tree.xpath(
            '//div[@class="p-note user-profile-bio mb-3 js-user-profile-bio f4"]/div/text()'
        )
        profile.bio = bio_elements[0] if bio_elements else None

        profile.location = tree.xpath('//span[@class="p-label"]/text()')
        profile.email = tree.xpath(
            '//li[@itemprop="email"]/a[@class="Link--primary"]/text()'
        )
        profile.website = (
            URL(tree.xpath('//li[@itemprop="url"]/a[@class="Link--primary"]/text()')[0])
            if tree.xpath('//li[@itemprop="url"]/a[@class="Link--primary"]/text()')
            else None
        )
        profile.twitter = (
            tree.xpath(
                '//li[@style="max-width: 230px"]/a[@class="Link--primary"]/text()'
            )[0]
            if tree.xpath(
                '//li[@style="max-width: 230px"]/a[@class="Link--primary"]/text()'
            )
        ...
        profile.linkedin = (
            tree.xpath(
                '//a[@class="Link--primary" and contains(@href, "linkedin.com")]/text()'
            )[0]
            if tree.xpath(
                '//a[@class="Link--primary" and contains(@href, "linkedin.com")]/text()'
            )
        ...
        profile.instagram = (
            tree.xpath(
                '//a[@class="Link--primary" and contains(@href, "instagram.com")]/text()'
            )[0]
            if tree.xpath(
                '//a[@class="Link--primary" and contains(@href, "instagram.com")]/text()'
            )
        ...
        profile.followers = (
            int(
                tree.xpath(
                    '//a[@class="Link--secondary no-underline no-wrap"]/span[@class="text-bold color-fg-default"]/text()'
                )[0]
            )
            if tree.xpath(
                '//a[@class="Link--secondary no-underline no-wrap"]/span[@class="text-bold color-fg-default"]/text()'
        ...
        profile.stars = (
            int(
                tree.xpath(
                    f'//a[@data-selected-links="stars /{profile.username}?tab=stars"]/span[@class="Counter"]/text()'
                )[0]
            )
            if tree.xpath(
                f'//a[@data-selected-links="stars /{profile.username}?tab=stars"]/span[@class="Counter"]/text()'
        ...
        profile.repositories = (
            int(
                tree.xpath(
                    f'//a[@data-selected-links="repositories /{profile.username}?tab=repositories"]/span[@class="Counter"]/text()'
                )[0]
            )
            if tree.xpath(
                f'//a[@data-selected-links="repositories /{profile.username}?tab=repositories"]/span[@class="Counter"]/text()'
        ...
        created_at_xpath_result = tree.xpath(
            '//time[@class="d-inline-block mb-2"]/text()'
        )
        profile.created_at = (
            datetime.strptime(created_at_xpath_result[0], "on %B %d, %Y")
            if created_at_xpath_result
            else None
        )
        profile.contributions = (
            int(
                tree.xpath('//h2[@class="f4 text-normal mb-2"]/text()')[0]
                .strip()
                .split()[0]
            )
            if tree.xpath('//h2[@class="f4 text-normal mb-2"]/text()')
            else 0
        )
        profile.organizations = tree.xpath('//a[@class="orgs-member"]')
        profile.pinned_repositories = tree.xpath(
