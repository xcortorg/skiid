...
class Information(Cog):
    ...
    async def userinfo_slash(
        ...
        embed = Embed(
            color=user.color if user.color != Colour.default() else Colour.default()
        )
            ...
            if EMOJIS.BADGES.BOOST not in badges:
                for guild in user.mutual_guilds:
                    ...
                    if member.premium_since:
                        ...
                        break
            donator = await interaction.client.db.fetchrow(
                "SELECT badge FROM donators WHERE user_id = $1", user.id
            )
    ...
    async def botinfo(self, ctx: Context) -> Message:
        """
        View information such as commands, guilds, and overall users for the bot.
        """

        total_guilds = await self.bot.get_guilds()
        total_members = await self.bot.get_users()
        total_commands = len(set(self.bot.walk_commands()))
        total_cogs = len(self.bot.cogs)

        embed = Embed(
            description=(
                f"Serving `{total_guilds:,}` guilds with `{total_members:,}` users\n"
                f"Utilizing `{total_commands:,}` commands across `{total_cogs:,}` cogs.\n"
                f"[greed vanity](https://discord.com/oauth2/authorize?client_id=1132607680163352627"
                f"&scope=bot+applications.commands&permissions=8) is now **LIVE** in open beta.\n"
                f"Hosting services provided by [Kuroit](https://my.kuroit.com/aff.php?aff=144)"
            )
        )
        ...
        memory_usage = format_size(process.memory_info().rss)
        uptime = self.bot.uptime
        cluster_id = os.getenv("CLUSTER")

        embed.add_field(
            name=f"**System** (Cluster ID: {cluster_id})",
            value="\n".join(
                [
    ...
    async def serverinfo(
        ...
        *,
        ...
        guild = guild or ctx.guild
        embed = Embed(
            description=f"{format_dt(guild.created_at)} ({format_dt(guild.created_at, 'R')})\n**__{guild.name}__** is on Cluster **{cluster_id}** & Shard **{shard_id}**"
        )
        embed.set_author(
            name=f"{guild.name} ({guild.id})",
