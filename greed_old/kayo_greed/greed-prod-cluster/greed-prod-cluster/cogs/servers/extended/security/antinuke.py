...
class Settings(BaseModel):
    ...
    async def check_threshold(self, bot: greed, member: Member, module: str) -> bool:
        """
    ...
    @classmethod
    async def revalidate(cls, bot: greed, guild: Guild) -> Optional[Self]:
        """
    ...
    @classmethod
    async def fetch(cls, bot: greed, guild: Guild) -> Self:
        """
        ...
        try:
            await guild.ban(
                ...
                reason=(
                    ...
                    f"({config.webhook.threshold} webhooks in {config.webhook.duration})"
                ),
            )
        except HTTPException as exc:
            await config.dispatch_log(
