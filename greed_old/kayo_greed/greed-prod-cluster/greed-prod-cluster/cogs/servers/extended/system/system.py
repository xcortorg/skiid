...
class System(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def initialize_guild_data(self, guild_id: int) -> None:
        """Initialize locks and queues for a guild."""
        if guild_id not in self.locks:
            self.locks[guild_id] = asyncio.Lock()
        if guild_id not in self.join_queue:
            self.join_queue[guild_id] = []

    async def disable_ping_on_join(self, guild_id: int, ctx: Context) -> None:
        """Disable ping on join for a guild."""
        await self.bot.db.execute(
            "DELETE FROM pingonjoin WHERE guild_id = $1", guild_id
        )
        await ctx.send("Disabled ping on join for this server.")

    async def enable_ping_on_join(
        self, guild_id: int, channel: TextChannel, flags: Flags, ctx: Context
    ) -> None:
        """Enable ping on join for a guild with specified channel and flags."""
        await self.bot.db.execute(
            ...
            guild_id,
            channel.id,
            flags.delete_after,  # Correct attribute access
        )
        await ctx.send(
            f"Enabled ping on join for {channel.mention}. Delete after: {flags.delete_after} seconds."
        )

    async def fetch_pingonjoin_data(self, guild_id: int) -> dict | None:
        """Fetch pingonjoin data for a guild."""
    ...
    async def notify_failure(
        ...
        with suppress(HTTPException):
            return await owner.send(
                embed=embed,
                content=(codeblock(template) if len(template) > 1024 else None),
            )
    ...
    @has_permissions(manage_guild=True)
    async def poj(self, ctx: Context) -> None:
        """Base command group for managing ping on join settings."""
        await ctx.send_help(ctx.command)

    @poj.command(
        name="set",
        description="manage guild",
        brief="#giveaways --delete_after 3",
        usage="<channel> [flags]",
    )
    @has_permissions(manage_guild=True)
    async def poj_set(
        self, ctx: Context, channel: TextChannel, *, flags: Flags
    ) -> None:
        """Enable ping on join for a channel."""
        guild_id = ctx.guild.id
        await self.initialize_guild_data(guild_id)
        await self.enable_ping_on_join(guild_id, channel, flags, ctx)

    @poj.command(name="disable", description="manage guild")
    @has_permissions(manage_guild=True)
    async def poj_disable(self, ctx: Context) -> None:
        """Disable ping on join for the server."""
        guild_id = ctx.guild.id
        await self.initialize_guild_data(guild_id)
        await self.disable_ping_on_join(guild_id, ctx)

    async def handle_member_join(self, member: Member, data: dict) -> None:
        """Handle a new member joining the guild and send a ping message if configured."""
        guild_id = member.guild.id
        channel = member.guild.get_channel(data["channel_id"])

        # Ensure the channel is a TextChannel
        if not isinstance(channel, TextChannel):
            log.error(f"Channel {data['channel_id']} is not a text channel.")
            return
        ...
        async with self.locks[guild_id]:
            ...
            await asyncio.sleep(1)  # Debounce to handle rapid joins
            mentions = " ".join(self.join_queue[guild_id])
            self.join_queue[guild_id].clear()

            try:
                message = await channel.send(mentions)
    ...
    @Cog.listener("on_member_join")
    async def on_pingonjoin(self, member: Member) -> None:
        """Send a ping message to a channel when a new member joins."""
        guild_id = member.guild.id
        data = await self.fetch_pingonjoin_data(guild_id)
        if data:
            await self.handle_member_join(member, data)
