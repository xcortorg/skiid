...
def ListedFonts():
    fonts = []
    for file in os.listdir('structure/fonts/'):
...
async def quoter(message: discord.Message, font: str, color: Literal['white', 'black'] = None, contrast: bool = None, flip: bool = None, gif: bool = None, new: bool = None, blur: bool = None, brightness: bool = None, pixelate: bool = None, solarize: bool = None) -> Image:
    ...
    await asyncio.to_thread(FontDB.LoadFromPath, 'custom-font', f'structure/fonts/{font}')
...
class Buttons(View):
    ...
    def __init__(self, ctx, author):
        ...
        self.is_gif_enabled = False
        self.gif_button = None
        self.selected_font = self.font
        self.select_font.options = self._get_font_options()
    ...
    def _get_font_options(self):
        ...
        for font in ListedFonts():
            option = discord.SelectOption(
                label=font,
                value=font,
                emoji="<:blarrow:1341204214273146902>" if font == self.selected_font else None,
                default=font == self.selected_font
            )
            options.append(option)
        return options

    async def authorCheck(self, interaction: Interaction):
        if self.author != interaction.user:
            embed = await cembed(interaction, description=f"Only {self.author.mention} can use this button.")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return False
        return True

    async def UpdateImage(self, interaction: Interaction):
        try:
            image = await quoter(self.message, self.font,
                color='white' if self.color.style == discord.ButtonStyle.blurple else 'black',
                contrast=False if self.contrast.style == discord.ButtonStyle.blurple else True,
                flip=True if self.flip.style == discord.ButtonStyle.blurple else False,
                gif=False,
                new=True if self.new.style == discord.ButtonStyle.blurple else False,
                blur=True if self.blur.style == discord.ButtonStyle.blurple else False,
                brightness=True if self.brightness.style == discord.ButtonStyle.blurple else False,
                pixelate=True if self.pixelate.style == discord.ButtonStyle.blurple else False,
                solarize=True if self.solarize.style == discord.ButtonStyle.blurple else False
            )
            image_file = await GBytes(image, gif=self.is_gif_enabled)
            
            if interaction.response.is_done():
                await interaction.edit_original_response(view=self, attachments=[image_file])
            else:
                await interaction.response.edit_message(view=self, attachments=[image_file])
                
        except Exception as e:
            await self.send_error_message(interaction, str(e))

    async def send_error_message(self, interaction: Interaction, error_message: str):
        embed = await cembed(interaction, description=f"You **cannot** apply due to an error: {error_message}.")
        await interaction.response.send_message(embed=embed, ephemeral=True)

    async def button_check(self, interaction: Interaction):
        return await self.authorCheck(interaction)

    async def disable_buttons(self):
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                if self.is_gif_enabled:
                    child.disabled = child not in [self.gif_button, self.remove_quote]
                else:
    ...
    async def color(self, interaction: Interaction, button: Button):
        if self.is_gif_enabled:
            embed = await cembed(interaction, description=f"Disable GIF mode to edit this quote.")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        if not await self.button_check(interaction):
            return
        self.color.style = discord.ButtonStyle.grey if self.color.style == discord.ButtonStyle.blurple else discord.ButtonStyle.blurple
        await self.UpdateImage(interaction)

    @discord.ui.button(style=discord.ButtonStyle.grey, emoji=EMOJIS["contrast"])
    async def contrast(self, interaction: Interaction, button: Button):
        if self.is_gif_enabled:
            embed = await cembed(interaction, description=f"Disable GIF mode to edit this quote.")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        if not await self.button_check(interaction):
            return
        self.contrast.style = discord.ButtonStyle.grey if self.contrast.style == discord.ButtonStyle.blurple else discord.ButtonStyle.blurple
        await self.UpdateImage(interaction)

    @discord.ui.button(style=discord.ButtonStyle.grey, emoji=EMOJIS["flip"])
    async def flip(self, interaction: Interaction, button: Button):
        if self.is_gif_enabled:
            embed = await cembed(interaction, description=f"Disable GIF mode to edit this quote.")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        if not await self.button_check(interaction):
            return
        self.flip.style = discord.ButtonStyle.grey if self.flip.style == discord.ButtonStyle.blurple else discord.ButtonStyle.blurple
        await self.UpdateImage(interaction)

    @discord.ui.button(style=discord.ButtonStyle.grey, emoji=EMOJIS["gif"])
    async def gif(self, interaction: Interaction, button: Button):
        if not await self.button_check(interaction):
            return
            
        try:
            self.is_gif_enabled = not self.is_gif_enabled
            button.style = discord.ButtonStyle.grey if not self.is_gif_enabled else discord.ButtonStyle.blurple
            self.gif_button = button
            await self.disable_buttons()
            await self.UpdateImage(interaction)
        except Exception as e:
            await interaction.followup.send(f"An error occurred: {str(e)}", ephemeral=True)
            print(f'GYATTTTTTTTTTTTT {e}')
            self.is_gif_enabled = not self.is_gif_enabled
            button.style = discord.ButtonStyle.grey if not self.is_gif_enabled else discord.ButtonStyle.blurple
    ...
    async def new(self, interaction: Interaction, button: Button):
        ...
        if not await self.button_check(interaction):
            return
        self.new.style = discord.ButtonStyle.grey if self.new.style == discord.ButtonStyle.blurple else discord.ButtonStyle.blurple
        self.flip.disabled = True if self.new.style == discord.ButtonStyle.blurple else False
        await self.UpdateImage(interaction)

    @discord.ui.button(style=discord.ButtonStyle.grey, emoji=EMOJIS["blur"])
    async def blur(self, interaction: Interaction, button: Button):
        if self.is_gif_enabled:
            embed = await cembed(interaction, description=f"Disable GIF mode to edit this quote.")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        if not await self.button_check(interaction):
            return
        self.blur.style = discord.ButtonStyle.grey if self.blur.style == discord.ButtonStyle.blurple else discord.ButtonStyle.blurple
        await self.UpdateImage(interaction)

    @discord.ui.button(style=discord.ButtonStyle.grey, emoji=EMOJIS["brightness"])
    async def brightness(self, interaction: Interaction, button: Button):
        if self.is_gif_enabled:
            embed = await cembed(interaction, description=f"Disable GIF mode to edit this quote.")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        if not await self.button_check(interaction):
            return
        self.brightness.style = discord.ButtonStyle.grey if self.brightness.style == discord.ButtonStyle.blurple else discord.ButtonStyle.blurple
        await self.UpdateImage(interaction)

    @discord.ui.button(style=discord.ButtonStyle.grey, emoji=EMOJIS["pixelate"])
    async def pixelate(self, interaction: Interaction, button: Button):
        if self.is_gif_enabled:
            embed = await cembed(interaction, description=f"Disable GIF mode to edit this quote.")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        if not await self.button_check(interaction):
            return
        self.pixelate.style = discord.ButtonStyle.grey if self.pixelate.style == discord.ButtonStyle.blurple else discord.ButtonStyle.blurple
        await self.UpdateImage(interaction)

    @discord.ui.button(style=discord.ButtonStyle.grey, emoji=EMOJIS["solarize"])
    async def solarize(self, interaction: Interaction, button: Button):
        if self.is_gif_enabled:
            embed = await cembed(interaction, description=f"Disable GIF mode to edit this quote.")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        if not await self.button_check(interaction):
            return
        self.solarize.style = discord.ButtonStyle.grey if self.solarize.style == discord.ButtonStyle.blurple else discord.ButtonStyle.blurple
        await self.UpdateImage(interaction)

    @discord.ui.button(emoji=EMOJIS["remove"], style=discord.ButtonStyle.grey)
    async def remove_quote(self, interaction: Interaction, button: Button):
        if not await self.button_check(interaction):
            return
    ...
    async def select_font(self, interaction: Interaction, select: discord.ui.Select):
        if self.is_gif_enabled:
            embed = await cembed(interaction, description=f"Disable GIF mode to edit this quote.")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        self.selected_font = select.values[0]
        self.font = self.selected_font

        for option in select.options:
            option.emoji = "<:blarrow:1341204214273146902>" if option.value == self.selected_font else None
            option.default = option.value == self.selected_font

        await self.UpdateImage(interaction)
        await interaction.response.edit_message(view=self)

class SavePresetModal(Modal):
    def __init__(self, embed_data, user_id):
        super().__init__(title="Save Embed Preset")
        self.embed_data = embed_data
        self.user_id = user_id
        self.preset_name_input = TextInput(
            label="Enter a name for this preset",
            placeholder="My Custom Embed",
            max_length=255,
            required=True,
            style=discord.TextStyle.short
        )
        self.add_item(self.preset_name_input)
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        async with get_db_connection() as conn:
            existing_preset = await conn.fetchval(
                "SELECT COUNT(*) FROM embed_presets WHERE user_id = $1 AND preset_name = $2",
                user_id, preset_name
            )
            
            if existing_preset > 0:
                await interaction.response.send_message(
                    "You already saved a preset under this name, if you wish to edit it, use </selfembed edit:123> instead.",
                    ephemeral=True
                )
                return
            ...
            if current_presets >= max_presets:
                await interaction.response.send_message(
                    f"You've reached your maximum limit of {max_presets} presets. "
                    f"{'Delete some existing presets to save new ones.' if not is_donor else ''}"
                    f"{'' if not is_donor else ' Upgrade to Premium for up to 10 presets! </premium buy:1278389799857946700>'}",
                    ephemeral=True
                )
                return

            await conn.execute(
                """INSERT INTO embed_presets (
                    user_id, preset_name, title, description, footer, author, footer_image,
                    image, thumbnail, author_image, color, timestamp
                ...
                self.embed_data['footer_image'],
                self.embed_data['image'],
                self.embed_data['thumbnail'],
                self.embed_data['author_image'],
                self.embed_data['color'],
                self.embed_data['timestamp']
            )
            remaining_slots = max_presets - (current_presets + 1)
            await interaction.response.send_message(
                f"Embed preset '{preset_name}' has been saved. You have {remaining_slots} preset slot{'s' if remaining_slots != 1 else ''} remaining.",
                ephemeral=True
            )

class EmbedBuilderView(View):
    def __init__(self, user_id, footer_text=None):
        ...
        self.user_id = user_id
        self.embed = Embed()
        self.embed.set_footer(text=self.embed_data["footer"])

    @discord.ui.button(label="Title", style=ButtonStyle.primary)
    async def title_button(self, interaction: Interaction, button: Button):
        await interaction.response.send_modal(EditTextModal(self.embed_data, "title", "Enter Title", "Title", self))

    @discord.ui.button(label="Description", style=ButtonStyle.primary)
    async def description_button(self, interaction: Interaction, button: Button):
        await interaction.response.send_modal(EditTextModal(self.embed_data, "description", "Enter Description", "Description", self))

    @discord.ui.button(label="Footer text", style=ButtonStyle.primary)
    async def footer_button(self, interaction: Interaction, button: Button):
        await interaction.response.send_modal(EditTextModal(self.embed_data, "footer", "Enter Footer Text", "Footer", self))

    @discord.ui.button(label="Author text", style=ButtonStyle.primary)
    async def author_button(self, interaction: Interaction, button: Button):
        await interaction.response.send_modal(EditTextModal(self.embed_data, "author", "Enter Author Text", "Author", self))
        await self.update_author_button()

    @discord.ui.button(label="Footer image", style=ButtonStyle.primary)
    async def footer_image_button(self, interaction: Interaction, button: Button):
        await interaction.response.send_modal(EditTextModal(self.embed_data, "footer_image", "Enter Footer Image URL", "Footer Image", self))

    @discord.ui.button(label="Author image", style=ButtonStyle.primary)
    async def author_image_button(self, interaction: Interaction, button: Button):
        await interaction.response.send_modal(EditTextModal(self.embed_data, "author_image", "Enter Author Image URL", "Author Image", self))

    @discord.ui.button(label="Image", style=ButtonStyle.primary)
    async def image_button(self, interaction: Interaction, button: Button):
        await interaction.response.send_modal(EditTextModal(self.embed_data, "image", "Enter Image URL", "Image", self))

    @discord.ui.button(label="Thumbnail", style=ButtonStyle.primary)
    async def thumbnail_button(self, interaction: Interaction, button: Button):
        await interaction.response.send_modal(EditTextModal(self.embed_data, "thumbnail", "Enter Thumbnail URL", "Thumbnail", self))

    @discord.ui.button(label="Color", style=ButtonStyle.primary)
    async def color_button(self, interaction: Interaction, button: Button):
        await interaction.response.send_modal(EditTextModal(self.embed_data, "color", "Enter Color (Hex)", "Color", self))

    @discord.ui.button(label="Save", style=ButtonStyle.green)
    async def save_button(self, interaction: Interaction, button: Button):
        await interaction.response.send_modal(SavePresetModal(self.embed_data, self.user_id))
    ...
    async def update_embed(self):
        self.embed = Embed(
            ...
            color=int(self.embed_data["color"].lstrip('#'), 16) if self.embed_data["color"] else None
        )
        if self.embed_data["footer"]:
            self.embed.set_footer(text=self.embed_data["footer"], icon_url=self.embed_data["footer_image"])
        if self.embed_data["author"]:
            self.embed.set_author(name=self.embed_data["author"], icon_url=self.embed_data["author_image"])
        if self.embed_data["image"]:
            self.embed.set_image(url=self.embed_data["image"])
        if self.embed_data["thumbnail"]:
            self.embed.set_thumbnail(url=self.embed_data["thumbnail"])
        if self.embed_data["timestamp"]:
            self.embed.timestamp = discord.utils.utcnow()

class EditTextModal(Modal):
    def __init__(self, embed_data, field, label, title, view):
    ...
    async def on_submit(self, interaction: Interaction):
        self.embed_data[self.field] = self.text_input.value.replace('\\n', '\n')
        await self.view.update_embed()
        await interaction.response.edit_message(embed=self.view.embed)

class EditPresetModal(Modal):
    def __init__(self, embed_data, preset_id, preset_name):
        super().__init__(title=f"Save Changes to {preset_name}")
        self.embed_data = embed_data
    ...
    async def on_submit(self, interaction: Interaction):
        async with get_db_connection() as conn:
            await conn.execute(
                """UPDATE embed_presets SET 
                title = $1, description = $2, footer = $3, author = $4, 
                footer_image = $5, image = $6, thumbnail = $7, author_image = $8,
                color = $9, timestamp = $10
                WHERE id = $11""",
                self.embed_data['title'],
                self.embed_data['description'],
                self.embed_data['footer'],
            ...
            await interaction.response.send_message(f"Your changes have been saved.", ephemeral=True)

class EditEmbedBuilderView(EmbedBuilderView):
    def __init__(self, user_id, preset_id, preset_name):
        super().__init__(user_id)
        self.preset_id = preset_id
        self.preset_name = preset_name

    @discord.ui.button(label="Save Changes", style=ButtonStyle.green)
    async def save_button(self, interaction: Interaction, button: Button):
        async with get_db_connection() as conn:
            await conn.execute(
                """UPDATE embed_presets SET 
            ...
            await interaction.response.send_message(f"Changes to '{self.preset_name}' have been saved.", ephemeral=True)

class Utility(commands.Cog):
    def __init__(self, client):
        self.client = client
        self.translator = Translator()
        FontDB.LoadFromDir("/heist/structure/fonts")
        ...
        self.client.tree.add_command(self.ctx_quote)
    ...
    async def cog_unload(self) -> None:
        ...
        self.client.tree.remove_command(self.ctx_quote.name, type=self.ctx_quote.type)
    ...
    @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    @app_commands.check(permissions.is_blacklisted)
    async def quotemessage(self, ctx: Interaction, message: discord.Message):
        try:
            if not message.content:
                if message.embeds:
                    await ctx.response.send_message("You cannot quote an embed.", ephemeral=True)
                    return
                elif message.attachments:
                    await ctx.response.send_message("You cannot quote a message containing Only images/files.", ephemeral=True)
                    return
                elif message.stickers:
                    await ctx.response.send_message("You cannot quote a message containing stickers.", ephemeral=True)
                    return
                else:
                    await ctx.response.send_message("You cannot quote this message.", ephemeral=True)
                    return

            if not message.content:
                return await ctx.response.send_message("No content to quote.", ephemeral=True)

            if not ctx.app_permissions.attach_files:
                await ctx.response.defer(thinking=True, ephemeral=True)
            else:
                await ctx.response.defer(thinking=True)

            view = Buttons(ctx, author=ctx.user)
            view.message = message
            view.font = 'M PLUS Rounded 1c (mplus).ttf'
            image = await quoter(view.message, view.font, color='black', contrast=True)
            await ctx.followup.send(file=await GBytes(image), view=view)
        except Exception as e:
            print(e)
