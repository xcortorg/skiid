...
from ...apis.playlist_interface import (Playlist, create_playlist,
                                        delete_playlist, get_all_playlist)
...
from ..cog_utils import (CompositeMetaClass, LazyGreedyConverter,
                         PlaylistConverter)
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            if to_append and to_append_count == 1:
                ...
                if to in tracks_obj_list:
                    return await self.send_embed_msg(
                        ...
                        footer=(
                            _("Playlist limit reached: Could not add track.").format(
                                not_added
                            )
                            if not_added > 0
                            else None
            ...
            await self.send_embed_msg(
                ...
                embed=embed,
                footer=(
                    _("Playlist limit reached: Could not add track.").format(not_added)
                    if not_added > 0
                    else None
                ),
            )
    ...
    async def command_playlist_queue(
        ...
        **Guild** can be one of the following:
        ...
        await self.send_embed_msg(
            ...
            footer=(
                _("Playlist limit reached: Could not add {} tracks.").format(not_added)
                if not_added > 0
                else None
            ),
        )
