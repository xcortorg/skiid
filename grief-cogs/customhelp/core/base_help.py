...
import discord
from customhelp.core.views import (BaseInteractionMenu, ReactButton,
                                   SelectArrowHelpBar, SelectMenuHelpBar)

from grief.core import commands
from grief.core.commands.commands import Command
from grief.core.commands.context import Context
from grief.core.commands.help import (HelpSettings, NoCommand, NoSubCommand, _,
                                      dpy_commands)
...
from .utils import (get_aliases, get_category_page_mapper_chunk, get_cooldowns,
                    get_perms, shorten_line)
                ...
                if self.settings["arrowtype"] == "emojis":
                    ...
                    for arrow in ARROWS:
                        ...
                        if arrow.name == "cross":
                            ...
                            continue
                        if self.settings["nav"]:
                            await self.menus[0].add_button(
                                await arrow_react(arrow), react=True
                            )
