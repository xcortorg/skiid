    ...
    async def callback(self, interaction, button):
        ...
        if interaction.user in self.joined:
            await interaction.response.send_message(
                _("You already joined this game! Wait for it to start."), ephemeral=True
            )
            return
        await interaction.response.send_message(
            _("You have joined the game! Please wait for it to begin."), ephemeral=True
        )
...
class PartyGames(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.config.register_guild(
            locale=None, timeBomb=7, timeFast=15, timeLong=15, timeMost=15
        )
        self.waiting = {}
        self.games = []
    ...
    async def bombparty(self, ctx, hp: int = 3):
        ...
        while game:
            for p in players:
                ...
                await ctx.send(
                    _("{p}, type a word containing: **{char}**").format(
                        p=p.mention, char=c
                    )
                )
                try:
                    ...
                    health[p.id] -= 1
                    await ctx.send(
                        _("Time's up! -1 HP ({health} remaining)").format(
                            health=health[p.id]
                        )
                    )
                    if health[p.id] == 0:
    ...
    async def fast(self, ctx, maxpoints: int = 5):
        ...
        afk = 0
        while game:
            score, used, mem = await self._fast(
                ctx, score, used, players, wordlist, locale
            )
            if mem is None:
                afk += 1
                if afk == 3:
                    await ctx.send(
                        _("No one wants to play :(\n{board}").format(
                            board=self._make_leaderboard(ctx, score)
                        )
                    )
                    game = False
                else:
                ...
                if score[mem.id] >= maxpoints:
                    await ctx.send(
                        _("{mem} wins!\n{board}").format(
                            mem=mem.mention, board=self._make_leaderboard(ctx, score)
                        )
                    )
                    game = False
            await asyncio.sleep(3)
    ...
    async def _fast(self, ctx, score, used, players, wordlist, locale):
        c = random.choice(CHARS[locale])
        await ctx.send(
            _("Be the first person to type a word containing: **{char}**").format(
                char=c
            )
        )
        try:
            ...
            await ctx.send(
                _("{mem} gets a point! ({score} total)").format(
                    mem=word.author.mention, score=score[word.author.id]
                )
            )
            used.append(word.content.lower())
    ...
    async def long(self, ctx, maxpoints: int = 5):
        ...
        afk = 0
        while game:
            score, used, mem = await self._long(
                ctx, score, used, players, wordlist, locale
            )
            if mem is None:
                afk += 1
    ...
    async def _long(self, ctx, score, used, players, wordlist, locale):
        ...
        await ctx.send(
            _("{mem} gets a point! ({score} total)").format(
                mem=resultdict["bestmem"].mention, score=score[resultdict["bestmem"].id]
            )
        )
    ...
    async def most(self, ctx, maxpoints: int = 5):
        ...
        afk = 0
        while game:
            score, used, mem = await self._most(
                ctx, score, used, players, wordlist, locale
            )
            if mem is None:
                afk += 1
    ...
    async def _most(self, ctx, score, used, players, wordlist, locale):
        ...
        order = list(
            reversed(
                sorted(resultdict["pdict"], key=lambda m: len(resultdict["pdict"][m]))
            )
        )
        ...
        if len(winners) == 1:
            ...
            await ctx.send(
                _("{mem} gets a point! ({score} total)").format(
                    mem=self._get_name_string(ctx, order[0], True),
                    score=score[order[0]],
                )
            )
        ...
        while game:
            ...
            if g == 3:
                for p in players:
                    c = random.choice(CHARS[locale])
                    await ctx.send(
                        _("{mem}, type a word containing: **{char}**").format(
                            mem=p.mention, char=c
                        )
                    )
                    try:
                        word = await self.bot.wait_for(
                        ...
                        await ctx.send(
                            _("{mem} gets a point! ({score} total)").format(
                                mem=p.mention, score=score[p.id]
                            )
                        )
    ...
    async def locale(self, ctx, locale: str):
        ...
        await ctx.send(
            _("Locale override is now set to `{locale}`.").format(locale=locale)
        )
        ...
        if message.channel.id in self.waiting:
            if self.waiting[message.channel.id]["type"] == "long":
                if message.author.id in self.waiting[message.channel.id]["plist"]:
                    if (
                        self.waiting[message.channel.id]["chars"].lower()
                        in message.content.lower()
                        and message.content.lower()
                        in self.waiting[message.channel.id]["wordlist"]
                        and not message.content.lower()
                        in self.waiting[message.channel.id]["used"]
                        and len(message.content)
                        > len(self.waiting[message.channel.id]["best"])
                    ):
                        self.waiting[message.channel.id][
                            "best"
                ...
                if message.author.id in self.waiting[message.channel.id]["pdict"]:
                    if (
                        ...
                        self.waiting[message.channel.id]["used"].append(
                            message.content.lower()
                        )
                        self.waiting[message.channel.id]["pdict"][
                            message.author.id
                        ].append(message.content.lower())
                        await message.add_reaction("\N{WHITE HEAVY CHECK MARK}")
