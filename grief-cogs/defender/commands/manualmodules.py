...
from ..enums import Action, EmergencyMode, EmergencyModules, Rank
...
class ManualModules(MixinMeta, metaclass=CompositeMetaClass):  # type: ignore
    ...
    async def alert(self, ctx):
        ...
        if not await self.is_helper(ctx.author) and not await self.bot.is_mod(
            ctx.author
        ):
            ctx.command.reset_cooldown(ctx)
        ...
        if not notify_channel_id or not notify_channel:
            ...
            return await ctx.send(
                "I don't have a notify channel set or I could not find it."
            )
    ...
    async def vaporize(self, ctx, *members: discord.Member):
        ...
        if not override:
            ...
            if is_staff and not has_ban_perms:
                ...
                if em_enabled:
                    ...
                    return await ctx.send(
                        "You need ban permissions to use this module."
                    )
        ...
        if len(members) > max_targets:
            await ctx.send(
                f"No more than {max_targets} users at once. Please try again."
            )
            return
        for m in members:
        ...
        for m in members:
            try:
                await guild.ban(
                    m,
                    reason=f"Vaporized by {ctx.author} ({ctx.author.id})",
    ...
    )  # Only one concurrent session per guild
    @commands.guild_only()
    async def voteout(self, ctx, *, user: discord.Member):
        ...
        if not override:
            ...
            if is_staff and not has_action_perms:
                ...
                if em_enabled:
                    ...
                    return await ctx.send(
                        f"You need {perm_text} permissions to use this module."
                    )
        ...
        if Action(action) == Action.Ban:
            ...
            action_text = (
                "Votekicked with Defender."  # Softban can be considered a kick
            )
            reason = f"{action_text} Voters: {voters_list}"
