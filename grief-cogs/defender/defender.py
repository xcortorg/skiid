...
from .core.utils import timestamp, utcnow
...
from .enums import Action, EmergencyModules, PerspectiveAttributes, Rank
...
default_guild_settings = {
    ...
    "ca_attributes": [
        PerspectiveAttributes.SevereToxicity.value
    ],  # Attributes to query
...
class Defender(
    Commands, AutoModules, Events, commands.Cog, metaclass=CompositeMetaClass
):
        ...
        *,
        ...
        if isinstance(obj, (discord.Member, CacheUser)):
            ...
            async for m in AsyncIter(messages, steps=20):
                ...
                channel = guild.get_channel(m.channel_id) or guild.get_thread(
                    m.channel_id
                )
                ...
                if m.edits:
                    ...
                    for edit in m.edits:
                        ...
                        content = (
                            edit.content if requester_can_rm else text_unauthorized
                        )
                        _log.append(f"[{ts}]({channel})[{entry}] {content}")
                else:
    ...
    async def make_identify_embed(
        self, message: discord.Member, user, rank=True, link=True
    ):
        ...
        em.add_field(
            name="Account created", value=timestamp(user.created_at), inline=True
        )
        em.add_field(
            name="Joined this server", value=timestamp(user.joined_at), inline=True
        )
        if link:
            ...
            while True:
                ...
                async with members.all() as all_members:
                    for guid, counter in all_counters.items():
                        async for uid, n_messages in AsyncIter(
                            counter.items(), steps=50
                        ):
                            guid = str(guid)
                            uid = str(uid)
            ...
            for rule in rules.values():
                ...
                try:
                    ...
                    if new_rule.name is not None:
                        ...
                        log.error(
                            "Warden - rule did not reach name "
                            "parsing during cog load",
                            exc_info=e,
                        )
                except Exception as e:
                    if new_rule.name is not None:
                        self.invalid_warden_rules[int(guid)][new_rule.name] = new_rule  # type: ignore
                    log.error(
                        "Warden - unexpected error during cog load rule parsing",
                        exc_info=e,
                    )
                else:
    ...
    async def callout_if_fake_admin(self, ctx):
        ...
        has_basic_perms = all(
            (perms.manage_messages, perms.manage_roles, perms.ban_members)
        )
    ...
    async def send_notification(
        ...
        *,
        ...
        if send_embed is True and force_text_only is False:
            ...
            message_content = (
                f"{title}{staff_mention}{description}{fields_txt}{jump_to}{footer}"
            )
