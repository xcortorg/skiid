...
class HeatLevel:
    ...
    def _expire_heat(self):
        now = utcnow()
        self._heat_points = deque(
            [h for h in self._heat_points if h > now], maxlen=MAX_HEATPOINTS
        )
...
def increase_channel_heat(channel: discord.TextChannel, td: timedelta, *, debug=False):
    ...
    if heat:
        ...
        get_heat_store(channel.guild.id, debug)["channels"][channel.id].increase_heat(
            td
        )
...
def increase_custom_heat(guild: discord.Guild, key: str, td: timedelta, *, debug=False):
    ...
    if heat:
        ...
        get_heat_store(guild.id, debug)["custom"][key] = HeatLevel(
            guild.id, key, "custom"
        )
