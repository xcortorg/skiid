...
from ...core.utils import QuickAction, get_external_invite, utcnow
from ...core.warden import validation as models
from ...core.warden.validation import (ALLOWED_DEBUG_ACTIONS,
                                       ALLOWED_STATEMENTS, DEPRECATED,
                                       BaseModel, model_validator)
from ...enums import Action as ModAction
from ...enums import EmergencyMode, Rank
from ...exceptions import (ExecutionError, InvalidRule, MisconfigurationError,
                           StopExecution)
from . import heat
from .enums import (Action, ChecksKeys, Condition, ConditionalActionBlock,
                    ConditionBlock, Event)
from .utils import (REMOVE_C_EMOJIS_RE, delete_message_after,
                    has_x_or_more_emojis, make_fuzzy_suggestion,
                    run_user_regex)
        ...
        self.state.update(
            {
                ...
                "notification_channel_id": (
                    await cog.config.guild(guild).notify_channel() if cog else 0
                ),
            }
        )
            ...
            self.state.update(
                {
                    ...
                    "channel_category_id": (
                        channel.category.id if channel.category else "0"
                    ),
                ...
                self.state.update(
                    {
                        ...
                        "parent_heat": heat.get_channel_heat(
                            channel.parent, debug=self.debug
                        ),
                    }
                )
...
class WardenRule:
    ...
    async def parse(self, rule_str, cog: MixinMeta, author=None):
        ...
        if Event.Periodic in self.events:
            ...
            if "run-every" not in rule.keys():
                raise InvalidRule(
                    "The 'run-every' parameter is mandatory with " "periodic rules."
                )
            try:
                td = parse_timedelta(
                    str(rule["run-every"]),
    ...
    async def parse_tree(
        ...
        *,
        ...
        for _dict in raw_tree:
            ...
            try:
                if isinstance(enum, Condition):
                    ...
                    if outer_block is ConditionBlock:
                        raise InvalidRule(
                            "Actions are not allowed inside condition blocks"
                        )
                    model = model_validator(enum, value)
            ...
            if model and author:
                try:
                    await model._runtime_check(
                        cog=cog, author=author, action_or_cond=enum
                    )
            ...
            if model:
                for event in events:
                    if not enum in ALLOWED_STATEMENTS[event]:
                        raise InvalidRule(
                            self.errors["NOT_ALLOWED_IN_EVENTS"].format(enum.value)
                        )
        ...
        for statement, value in tree.items():
            ...
            if isinstance(statement, WDCondition):
                ...
                await self._do_action(
                    action=statement.enum, model=value, runtime=runtime
                )
            elif isinstance(statement, WDConditionBlock):
                block_bool_stop = statement.enum in (
                    ConditionBlock.IfNot,
                    ConditionBlock.IfAny,
                )
                ...
                can_run_true = (
                    statement.enum is ConditionalActionBlock.IfTrue
                    and runtime.last_result is True
                )
                can_run_false = (
    ...
    async def _evaluate_condition(
            ...
            if parent is None:  # Name matching for channels only
                for channel_str in params.value:
                    ...
                    channel_obj = discord.utils.get(
                        guild.text_channels, name=channel_str
                    )
        ...
        async def message_contains_invite(params: models.IsBool):
            ...
            if results:
                ...
                try:
                    ...
                    error_text = (
                        "Unexpected error: failed to fetch server's own invites"
                    )
        ...
        async def message_contains_mt_emojis(params: models.IsInt):
            over_limit = has_x_or_more_emojis(
                cog.bot, guild, message.content, params.value + 1
            )
            return over_limit
    ...
    async def _do_action(self, action: Action, *, model: BaseModel, runtime: WDRuntime):
        ...
        async def notify_staff(params: models.NotifyStaff):
            ...
            if params.jump_to:
                ...
                try:
                    jump_to_ch = discord.utils.get(
                        guild.text_channels, id=int(jump_to_channel_id)
                    )
                except ValueError:
                    raise ExecutionError(
                ...
                if jump_to_ch:
                    try:
                        jump_to_msg = jump_to_ch.get_partial_message(
                            int(jump_to_message_id)
                        )
                    except ValueError:
                        raise ExecutionError(
        ...
        async def add_roles_to_user(params: models.NonEmptyList):
            ...
            if to_assign:
                await user.add_roles(
                    *to_assign, reason=f"Assigned by Warden rule '{self.name}'"
                )
        ...
        async def set_user_nickname(params: models.IsStr):
            ...
            await user.edit(
                nick=value, reason=f"Changed nickname by Warden rule '{self.name}'"
            )
        ...
        async def punish_user(params: models.IsNone):
            ...
            if punish_role and not cog.is_role_privileged(punish_role):
                await user.add_roles(
                    punish_role, reason=f"Punished by Warden rule '{self.name}'"
                )
            else:
                cog.send_to_monitor(
                    guild,
            ...
            if runtime.last_sent_message is not None:
                cog.loop.create_task(
                    delete_message_after(
                        runtime.last_sent_message, params.value.seconds
                    )
                )
        ...
        async def send_message(params: models.SendMessage):
            ...
            if send_embed is False and not params.content:
                raise ExecutionError(
                    f"[Warden] ({self.name}): I have no content and "
                    "no embed to send."
                )
            ...
            if not params.edit_message_id:
                ...
                try:
                    partial_msg = destination.get_partial_message(
                        int(params.edit_message_id)
                    )
        ...
        async def var_math(params: models.VarMath):
            ...
            if op not in ops and op not in single_ops:
                ...
                raise ExecutionError(
                    f"A second operand is not needed with operator {op}"
                )

            num1, num2 = safe_sub(params.operand1), (
        ...
        async def warnsystem_warn(params: models.WarnSystemWarn):
            ...
            if ws is None:
                raise ExecutionError(
                    "WarnSystem is not loaded. Integration not available."
                )
            ...
            for rt in raw_targets:
                ...
                if member is None:
                    if rt.isnumeric():
                        raise ExecutionError(
                            "The hackban feature is not yet available."
                        )  # TODO
...
class WardenCheck(WardenRule):
    ...
    async def parse(self, rule_str, cog: MixinMeta, module: ChecksKeys, author=None):
        ...
        if not isinstance(rule, list):
            raise InvalidRule(
                f"Please review the format: checks should be a list of conditions"
            )
