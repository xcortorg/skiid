...
from pydantic import (ExtraError, ValidationError, conint, conlist,
                      root_validator, validator)
from pydantic.error_wrappers import ErrorWrapper

from grief.core.commands.converter import BadArgument, parse_timedelta
...
def model_validator(
    action_or_cond: Union[Action, Condition],
    parameter: Union[list, dict, str, int, bool],
) -> BaseModel:
    """
        ...
        if isinstance(parameter, list):
            if len(parameter) > len(validator._short_form):
                raise ValidationError(
                    [ErrorWrapper(ExtraError(), loc="Short form")], validator
                )
            params = parameter
        else:
            params = (parameter,)
