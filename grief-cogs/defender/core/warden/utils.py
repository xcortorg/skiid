...
def has_x_or_more_emojis(
    bot: discord.Client, guild: discord.Guild, text: str, limit: int
):
...
async def run_user_regex(*, rule_obj, cog, guild: discord.Guild, regex: str, text: str):
    ...
    if safety_checks_enabled:
        try:
            ...
            log.error(
                "Warden - Unexpected error while running user defined regex", exc_info=e
            )
            return False
        else:
            return bool(result)
...
async def rule_add_periodic_prompt(*, cog, message: discord.Message, new_rule):
    ...
    async with channel.typing():
        ...
        async for m in AsyncIter(guild.members, steps=2):
            ...
            if await new_rule.satisfies_conditions(
                rank=rank, user=m, guild=guild, cog=cog
            ):
                affected += 1
