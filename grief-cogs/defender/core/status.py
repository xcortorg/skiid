...
async def make_status(ctx, cog):
    ...
    if not guild.me.guild_permissions.manage_guild:
        if await cog.config.guild(guild).invite_filter_enabled():
            exclude_own = await cog.config.guild(
                guild
    ...
    if not has_core_roles_set:
        msg += (
            f"**Configuration issue:** Core admin / mod roles are not set: see {p}set showsettings / "
            f"{p}help set\n"
        )
    if not n_channel:
    ...
    if not can_kick:
        msg += (
            "**Possible configuration issue:** I'm not able to kick in this server.\n"
        )
    if not can_read_al:
        msg += (
    ...
    em.add_field(
        name="Notify role", value=n_role.mention if n_role else "None set", inline=True
    )
    em.add_field(
        name="Notify channel",
        value=n_channel.mention if n_channel else "None set",
        inline=True,
    )
    em.add_field(
        name="Punish role",
        value=punish_role.mention if punish_role else "None set",
        inline=True,
    )
    ...
    if ca_action == Action.Ban:
        if ca_wipe:
            ...
            ca_action = (
                f"**ban** the author and **delete {ca_wipe} days** worth of messages"
            )
        else:
    ...
    msg += (
        f"{WD_CHECKS.format(is_active(await cog.config.guild(guild).ca_wdchecks()))}\n"
    )
    msg += "This module is currently "
    msg += "**enabled**.\n\n" if enabled else "**disabled**.\n\n"
    ...
    em = discord.Embed(color=discord.Colour.red(), description=msg)
    em.set_footer(
        text=f"`{p}dset warden` `{p}def warden` `{p}dset commentanalysis` to configure."
    )
    ...
    if EmergencyModules.Vaporize.value in em_modules:
        msg += (
            "It is set to be rendered available to helper roles in *emergency mode*.\n"
        )
    else:
