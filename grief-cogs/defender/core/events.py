...
from ..core.warden.enums import ChecksKeys as WDChecksKeys
from ..core.warden.enums import Event as WardenEvent
from ..core.warden.rule import WardenRule
from ..enums import Action, QAAction, Rank
...
class Events(MixinMeta, metaclass=CompositeMetaClass):  # type: ignore
    ...
    async def on_message(self, message: discord.Message):
            ...
            for rule in rules:
                if await rule.satisfies_conditions(
                    ...
                    try:
                        ...
                        log.error(
                            "Warden - unexpected error during actions execution",
                            exc_info=e,
                        )
        ...
        if inv_filter_enabled and not is_staff:
            ...
            if rank >= inv_filter_rank and await WardenAPI.eval_check(
                guild=guild,
                module=WDChecksKeys.InviteFilter,
                message=message,
                user=message.author,
            ):
                try:
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        if await self.config.guild(guild).warden_enabled():
            ...
            for rule in rules:
                ...
                if await rule.satisfies_conditions(
                    cog=self, rank=rank, guild=guild, user=member
                ):
                    try:
        ...
        for rule in rules:
            ...
            if await rule.satisfies_conditions(
                ...
                try:
                    ...
                    log.error(
                        "Warden - unexpected error during actions execution", exc_info=e
                    )
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        if action == Action.Ban:
            ...
            self.dispatch_event(
                "member_remove", target, action.value, quick_action.reason
            )
