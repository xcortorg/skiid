...
from patches.classes import BlackTea, RockPaperScissors, TicTacToe
...
class fun(commands.Cog):
        ...
        if member.id in self.bot.owner_ids:
            await ctx.reply(
                embed=discord.Embed(
                    color=self.bot.color,
                    title="how retarded",
                    description=f"{member.mention} is 0% retarded <a:retard:1216970788259893381>",
                )
            )
        else:
            await ctx.reply(
                embed=discord.Embed(
                    color=self.bot.color,
                    title="how retarded",
                    description=f"{member.mention} is `{randrange(101)}%` retarded <a:retard:1216970788259893381>",
                )
            )
        ...
        if member.id in self.bot.owner_ids:
            ...
            await ctx.reply(
                embed=discord.Embed(
                    color=self.bot.color,
                    title="gay r8",
                    description=f"{member.mention} is `{randrange(1000)}%` gay 🏳️‍🌈",
                )
            )
        ...
        if member.id in self.bot.owner_ids:
            ...
            await ctx.reply(
                embed=discord.Embed(
                    color=self.bot.color,
                    title="cool r8",
                    description=f"{member.mention} is `{randrange(1000)}%` cool 😎",
                )
            )
        ...
        if member.id in self.bot.owner_ids:
            ...
            await ctx.reply(
                embed=discord.Embed(
                    color=self.bot.color,
                    title="iq test",
                    description=f"{member.mention} has `{randrange(3000)}` iq 🧠",
                )
            )
    ...
    async def impersonate(self, ctx, user: discord.User, *, content):
        try:
            webhook = await ctx.channel.create_webhook(
                name=user.name, reason=f"{ctx.author} used mock command"
            )
            await ctx.message.delete()
            await webhook.send(
                str(content), username=user.display_name, avatar_url=user.avatar.url
            )
            await webhook.delete(reason=f"{ctx.author} used mock command")
        except discord.Forbidden:
            await ctx.send_warning(
                "I don't have the 'manage_webhooks' permission in this channel."
            )
                ...
                async with session.post(
                    ...
                    embeds = [
                        discord.Embed(
                            ...
                            url="https://" + res.get("domain", ""),
                            color=res.get("color", discord.Color.default()),
                        ).set_image(url=res.get("url", ""))
                        for res in response
                    ]
                if len(embeds) == 0:
                    return await ctx.send_warning("Nothing found.")
    ...
    async def search(self, ctx: commands.Context, *, query: str):
        ...
        for page, chunk in enumerate(chunks):
            embed = (
                discord.Embed(color=self.bot.color, title="Search Results")
                .set_author(
