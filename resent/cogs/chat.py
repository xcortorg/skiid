...
class chat(commands.Cog):
    ...
    async def delete(self, ctx: commands.Context, *, toggle: str):
        ...
        if not key:
            return await ctx.send_warning(
                f"You don't have an autoresponse for `{toggle}`"
            )
        ...
        return await ctx.send_success(
            f"Successfully **deleted** the autoresponse for `{toggle}`"
        )
    ...
    async def add(self, ctx: commands.Context, *, message: str):
        ...
        return await ctx.send_success(
            f"Successfully **added** the autoresponse for `{autoresponse[0].lstrip()}` with response `{autoresponse[1].lstrip()}`"
        )
    ...
    async def autoreact_list(self, ctx: commands.Context):
        ...
        if len(check) == 0:
            return await ctx.send_warning("this server has no **autoreactions**")
        i = 0
        k = 1
        l = 0
        mes = ""
