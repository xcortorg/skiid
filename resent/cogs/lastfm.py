...
import discord
import patches.userhandler as userhandler
...
async def lastfm_message(ctx: commands.Context, content: str) -> discord.Message:
    return await ctx.reply(
        embed=discord.Embed(
            color=0xFF0000,
            description=f"> <:lastfm:1208398661961130064> {ctx.author.mention}: {content}",
        )
    )
...
class lastfm(commands.Cog):
    ...
    @lf_embed.command(
        name="steal",
        description="steal someone's custom lastfm embed (premium feature)",
        help="lastfm",
        usage="[member]",
    )
    @premium()
    ...
    async def lf_whoknows(self, ctx: commands.Context, *, artist: str = None):
        ...
        if len(rows) == 0:
            return await ctx.send_warning(f"no one has listened to {artist}")
        embeds = []
        embed = discord.Embed(
            color=self.bot.color,
            description="\n".join(rows),
    ...
    async def lf_chart(
        ...
        x = await self.session.get_json(
            "https://api.resent.dev/lastfm/chart", params=params, headers=headers
        )
