...
def has_admin():
    async def predicate(ctx: commands.Context) -> bool:
        ...
        if not admin:
            return await ctx.send_warning("You do not have **anti-nuke admin**")
        return True
...
class antinuke(commands.Cog):
    ...
    async def settings(self, ctx: commands.Context):
        embed = discord.Embed(
            title=f"Anti-Nuke Settings - {ctx.guild.name}", color=self.bot.color
        )
        embed.set_footer(
        ...
        for name in self.modules:
            ...
            embed.add_field(
                name=f"**{name}**: {Emojis.OFF if not module or not module.toggled else Emojis.ON}",
    ...
    async def whitelisted(self, ctx: commands.Context):
        ...
        if not whitelisted:
            return await ctx.send_warning(
                "No users are **whitelisted** in this server."
            )
    ...
    async def admins(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            title=f"Anti-Nuke Admins - {ctx.guild.name}", color=self.bot.color
        )
        embed.set_footer(
            icon_url=ctx.me.avatar.url,
            text=f"If you have any questions, please join our support server: {self.bot.support_server}",
    ...
    async def enable(self, ctx: commands.Context):
        ...
        if enabled:
            return await ctx.send_warning(
                "AntiNuke is already **enabled** in this server."
            )
