...
const calculateRemaining = (start: number, end: number) => {
    ...
    const remainingSeconds = Math.max(remaining / 1000, 0)
    
    console.log({
        remainingTime: {
            seconds: remainingSeconds,
            minutes: Math.floor(remainingSeconds / 60),
            readableEnd: new Date(endTime).toISOString(),
            readableNow: new Date(now).toISOString()
        }
    })
    ...
    useEffect(() => {
        async function formatBio() {
            if (profile?.bio) {
                ...
                try {
                    const formatted = await formatDiscordText(profile.bio, profile.colors, resolvedParams.slug)
        ...
        formatBio()
    }, [profile?.bio, profile?.colors, resolvedParams.slug])
                ...
                <motion.div
                    ...
                    <div className="relative">
                        ...
                        <div className="absolute top-4 right-4">
                            <button
                                onClick={() => window.location.href = `https://evict.bot/@${resolvedParams.slug}`}
                                                        ...
                                                        activity?.end && (
                                                            <div className="mt-2">
                                                                <div className="h-1 bg-white/10 rounded-full w-[200px] overflow-hidden">
                                                                    <motion.div
                                                                        key={`${activity.start}-${activity.end}-${currentActivity}`}
                                                                        className="h-full origin-left"
                                                                        style={{
                                                                            background: profile.profile_color === "gradient"
                                                                                ? getProfileColorStyle(profile)
                                                                                : profile.linear_color,
                                                                            width: "100%"
                                                                        }}
                                                                        initial={{
                                                                            scaleX: calculateProgress(activity.start, activity.end) / 100
                                                                        }}
                                                                        animate={{
                                                                            scaleX: 1
                                                                        }}
                                                                        transition={{
                                                                            duration: calculateRemaining(activity.start, activity.end),
                                                                            ease: "linear"
                                                                        }}
                                                                    />
                                                                </div>
