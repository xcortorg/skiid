...
import { User as UserIcon, Shield, Download } from 'lucide-react';
...
const User: React.FC = () => {
  // State for logs data
  const [blacklists, setBlacklists] = useState<BlacklistUser[]>([]);
  ...
  useEffect(() => {
    const fetchAllBlacklists = async () => {
      ...
      try {
        const blacklistsData = await fetchAllUserBlacklists();
        setBlacklists(blacklistsData);
      } catch (err) {
        console.error('Error fetching user blacklists:', err);
        setError('Failed to fetch user blacklists. Please try again.');
        setBlacklists([]);
      } finally {
        setIsLoading(false);
      }
  ...
  // Function to fetch all user blacklists
  const fetchAllUserBlacklists = async () => {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['User ID', 'Moderator ID', 'Duration', 'Reason', 'Date'];
    const csvRows = [
      headers.join(','),
    ...
    link.setAttribute('download', `user_blacklists_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="p-6">
      ...
      <PageHeader
        ...
        description="View all user blacklists"
      />
          ...
          <div className="bg-[#161616] rounded-xl shadow-md overflow-hidden">
            {blacklists.length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium mb-2">No User Blacklist Entries Found</h3>
                <p className="text-gray-400 max-w-md mx-auto">
                  There are currently no user blacklist entries in the database.
                </p>
              </div>
            )}
          </div>
