...
import { Package2, Shield, Download, User as UserIcon } from 'lucide-react';
import { format } from 'date-fns';
import { fetchWithHeaders } from '../../../../lib/api';
import PageHeader from '../../../../components/PageHeader';

// Types for blacklist data
interface BlacklistCog {
  user_id: string;
  moderator_id: string;
  cog: string;
...
const Cog: React.FC = () => {
  // State for logs data
  const [blacklists, setBlacklists] = useState<BlacklistCog[]>([]);
  ...
  useEffect(() => {
    const fetchAllBlacklists = async () => {
      ...
      try {
        const blacklistsData = await fetchAllCogBlacklists();
        setBlacklists(blacklistsData);
      } catch (err) {
        console.error('Error fetching cog blacklists:', err);
        setError('Failed to fetch cog blacklists. Please try again.');
        setBlacklists([]);
      } finally {
        setIsLoading(false);
      }
  ...
  // Function to fetch all cog blacklists
  const fetchAllCogBlacklists = async () => {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['User ID', 'Moderator ID', 'Cog', 'Duration', 'Reason', 'Date'];
    const csvRows = [
      headers.join(','),
    ...
    link.setAttribute('download', `cog_blacklists_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="p-6">
      ...
      <PageHeader
        icon={<Shield />}
        title="Cog Blacklist"
        description="View all cog blacklists"
      />
          ...
          <div className="bg-[#161616] rounded-xl shadow-md overflow-hidden">
            {blacklists.length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium mb-2">No Cog Blacklist Entries Found</h3>
                <p className="text-gray-400 max-w-md mx-auto">
                  There are currently no cog blacklist entries in the database.
                </p>
              </div>
            )}
          </div>
