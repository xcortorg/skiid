...
import { Package2, Shield, Download, User as UserIcon, Server as ServerIcon, Search } from 'lucide-react';
import { format } from 'date-fns';
import { fetchWithHeaders } from '../../../../lib/api';
import PageHeader from '../../../../components/PageHeader';

// Types for blacklist data
interface BlacklistServerCog {
  guild_id: string;
  moderator_id: string;
  cog: string;
...
const Cog: React.FC = () => {
  // State for logs data
  const [blacklists, setBlacklists] = useState<BlacklistServerCog[]>([]);
  ...
  useEffect(() => {
    const fetchAllBlacklists = async () => {
      ...
      try {
        const blacklistsData = await fetchAllServerCogBlacklists();
        setBlacklists(blacklistsData);
      } catch (err) {
        console.error('Error fetching server cog blacklists:', err);
        setError('Failed to fetch server cog blacklists. Please try again.');
        setBlacklists([]);
      } finally {
        setIsLoading(false);
      }
  ...
  // Function to fetch all server cog blacklists
  const fetchAllServerCogBlacklists = async () => {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Server ID', 'Moderator ID', 'Cog', 'Duration', 'Reason', 'Date'];
    const csvRows = [
      headers.join(','),
    ...
    link.setAttribute('download', `server_cog_blacklists_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="p-6">
      ...
      <PageHeader
        icon={<Package2 />}
        title="Server Cog Blacklist"
        description="View all server cog blacklists"
      />
          ...
          <div className="bg-[#161616] rounded-xl shadow-md overflow-hidden">
            {blacklists.length > 0 ? (
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-[#1a1a1a] text-gray-400 text-xs uppercase">
                        ...
                        <th className="py-3.5 px-4 text-left font-medium">Cog</th>
                        ...
                        <tr key={index} className="hover:bg-[#1d1d1d] transition-colors">
                          <td className="py-3.5 px-4 text-gray-200 font-medium">
                            <div className="flex items-center">
                              <ServerIcon className="mr-2 w-4 h-4 text-gray-400" />
                              {blacklist.guild_id}
                            </div>
                          </td>
                          ...
                          <td className="py-3.5 px-4">
                            <div className="flex items-center">
                              <Package2 className="mr-2 w-4 h-4 text-gray-400" />
                              {blacklist.cog}
                            </div>
                          </td>
                          <td className="py-3.5 px-4">
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium mb-2">No Server Cog Blacklist Entries Found</h3>
                <p className="text-gray-400 max-w-md mx-auto">
                  There are currently no server cog blacklist entries in the database.
                </p>
              </div>
            )}
          </div>
