...
import { Command as CommandIcon, Shield, Download, User as UserIcon, Server as ServerIcon, Search } from 'lucide-react';
import { format } from 'date-fns';
import { fetchWithHeaders } from '../../../../lib/api';
import PageHeader from '../../../../components/PageHeader';

// Types for blacklist data
interface BlacklistServerCommand {
  guild_id: string;
  moderator_id: string;
  command: string;
...
const Command: React.FC = () => {
  // State for logs data
  const [blacklists, setBlacklists] = useState<BlacklistServerCommand[]>([]);
  ...
  useEffect(() => {
    const fetchAllBlacklists = async () => {
      ...
      try {
        const blacklistsData = await fetchAllServerCommandBlacklists();
        setBlacklists(blacklistsData);
      } catch (err) {
        console.error('Error fetching server command blacklists:', err);
        setError('Failed to fetch server command blacklists. Please try again.');
        setBlacklists([]);
      } finally {
        setIsLoading(false);
      }
  ...
  // Function to fetch all server command blacklists
  const fetchAllServerCommandBlacklists = async () => {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Server ID', 'Moderator ID', 'Command', 'Duration', 'Reason', 'Date'];
    const csvRows = [
      headers.join(','),
    ...
    link.setAttribute('download', `server_command_blacklists_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="p-6">
      ...
      <PageHeader
        icon={<CommandIcon />}
        title="Server Command Blacklist"
        description="View all server command blacklists"
      />
          ...
          <div className="bg-[#161616] rounded-xl shadow-md overflow-hidden">
            {blacklists.length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium mb-2">No Server Command Blacklist Entries Found</h3>
                <p className="text-gray-400 max-w-md mx-auto">
                  There are currently no server command blacklist entries in the database.
                </p>
              </div>
            )}
          </div>
