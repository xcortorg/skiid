...
class EmbedScript:
    def __init__(self, script: str, is_voicemaster: bool = False):
        ...
        self.is_voicemaster: bool = is_voicemaster
        self._type: str = "text"
    ...
    async def resolve_variables(self, **kwargs):
        ...
        if lastfm := kwargs.get("lastfm"):
            self.script = (
                self.script.replace("{lastfm}", lastfm.name)
        ...
        if artist := kwargs.get("artist"):
            self.script = (
                self.script.replace("{artist}", escape_markdown(artist.name))
        ...
        if album := kwargs.get("album"):
            self.script = (
                self.script.replace("{album}", escape_markdown(album.name))
        ...
        if track := kwargs.get("track"):
            self.script = (
                self.script.replace("{track}", escape_markdown(track.name))
