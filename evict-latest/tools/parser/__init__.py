    ...
    async def send(
        ...
        **kwargs,
        ...
        if isinstance(channel, Context):
            webhook = await channel.get_webhook()
            webhook_kwargs = {
            ...
            if isinstance(channel.channel, Thread):
                webhook_kwargs["thread"] = channel.channel
                
            webhook_kwargs = {k: v for k, v in webhook_kwargs.items() if v is not None}
            
            return await webhook.send(**webhook_kwargs)

        if not self.embed:
            return await channel.send(content=self.content, view=self.view, **kwargs)
