    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if await interaction.client.redis.exists(key):
                return await interaction.warn("Please wait before sending another confession!")
            ...
            if blacklist:
                for record in blacklist:
                    if record['word'].lower() in self.children[0].value.lower():
                        return await interaction.warn("Your confession contains blacklisted words!")
            ...
            if check:
                ...
                if not channel:
                    ...
                    return await interaction.warn("The confession channel no longer exists. Please set a new confession channel.")
                ...
                if any(link in self.children[0].value for link in links):
                    return await interaction.warn("I can't send links, those things can be dangerous!!")
