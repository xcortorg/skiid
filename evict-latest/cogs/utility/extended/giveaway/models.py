...
class GiveawayEntry(BaseModel):
    ...
    async def get_entry_count(self, member: Member) -> int:
        """Get number of entries for a member"""
        entries = 1
        ...
        global_bonus = await self.bot.db.fetchval(
            """
    ...
    async def entrants(self, message: Message) -> List[tuple[Member, int]]:
        """Returns list of (member, entry_count) tuples"""
        ...
        async for member in reaction.users():
            ...
            entry_count = await self.get_entry_count(member)
            entrants.append((member, entry_count))
    ...
    async def draw_winners(self, message: Message) -> List[Member]:
        ...
        with suppress(ValueError):
            return sample(pool, min(self.winners, len(pool)))
    ...
    async def save(self, message: Message) -> None:
        await self.bot.db.execute(
            ...
            message.id, self.prize, self.emoji, self.winners,
            self.ends_at, self.created_at,
            self.required_roles, self.bonus_roles
        )

    @classmethod
    def from_record(cls, bot: "Evict", record) -> Self:
        return cls(
            bot=bot,
            ...
            bonus_roles=record.get("bonus_roles", {})
        )
