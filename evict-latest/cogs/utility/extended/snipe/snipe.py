...
class Snipe(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def snipe(self, ctx: Context, index: Optional[str] = "1") -> Message:
        ...
        message = await MessageSnipe.get(self.bot.redis, ctx.channel.id, index_num)
        ...
        ignored_ids = await self.bot.db.fetch(
            """
            SELECT user_id 
            FROM snipe.ignore 
            WHERE guild_id = $1
        ...
        if ignored_ids and message.user_id in [record['user_id'] for record in ignored_ids]:
            return await ctx.warn(f"**{message.user_name}** is immune to being sniped!")
        ...
        if config and message.user_id in config.get("ignored_ids", []):
            return await ctx.warn(f"**{message.user_name}** is immune to being sniped!")
                ...
                for word in config["words"]:
                    if word in message.content.lower():
                        return await ctx.send(
                            "That message probably shouldn't be sniped!"
                        )
